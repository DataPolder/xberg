...
impl From<&FormFieldValue> for Object {
    fn from(value: &FormFieldValue) -> Self {
        match value {
            ...
            // UTF-8 bytes, which a reader misreads as PDFDocEncoding (mojibake). ~keep
            FormFieldValue::Text(s) => Object::text_string(s),
            FormFieldValue::Boolean(b) => {
                // Checkboxes use /Yes or /Off names ~keep
                if *b {
...
///
/// ```ignore
/// use xberg_native_pdf::editor::ParentFieldConfig;
///
...
impl FormFieldWrapper {
    ...
    pub fn from_read(field: FormField, page_index: usize, object_ref: Option<ObjectRef>) -> Self {
        ...
        // field and it vanishes from the form. ~keep
        let field_type = match &field.field_type {
            ...
            // Signature / Unknown are not fill targets; leave untyped. ~keep
            _ => None,
        };
        Self {
            name,
    ...
    pub fn from_widget<W: FormFieldWidget>(widget: &W, page_index: usize) -> Self {
        let field_type = match widget.field_type() {
            ...
            "Btn" => FormFieldType::Checkbox, // Could be checkbox, radio, or button ~keep
            "Ch" => FormFieldType::ComboBox,  // Could be combo or list ~keep
        ...
        let config = WidgetConfig {
            ...
            widget_dict: HashMap::new(), // Will be built with page_ref on save ~keep
    ...
    pub fn from_parent_config(config: &ParentFieldConfig) -> Self {
        Self {
            ...
            page_index: 0, // Parent-only fields don't have a page ~keep
            modified: false,
            is_new: true,
            object_ref: None,
            ...
            modified_rect: None,
            modified_default_value: None, // Already in modified_value ~keep
            modified_max_length: None,
            modified_alignment: None,
            modified_default_appearance: None,
            modified_background_color: None,
    ...
    pub fn is_merged(&self) -> bool {
        if self.is_new {
            // Radio groups are not merged (multiple widgets per field) ~keep
        ...
        // indicating merged format ~keep
        true // Default to merged for simplicity ~keep
    }
    ...
    pub fn build_field_dict(&self, page_ref: ObjectRef) -> HashMap<String, Object> {
        ...
        if let Some(parent_ref) = self.parent_ref {
            ...
            // Use partial name instead of full name for child fields ~keep
        ...
        // #616 ~keep
            ...
            // field disappears after fill+save. Re-emit /FT (and /Ff if set). ~keep
    ...
    pub fn build_parent_dict(&self) -> HashMap<String, Object> {
        ...
        dict.insert("T".to_string(), Object::text_string(&self.partial_name)); // #616 ~keep
        ...
        if let Some(ref tooltip) = self.modified_tooltip {
            dict.insert("TU".to_string(), Object::text_string(tooltip)); // #616 ~keep
        }
...
mod tests {
    ...
    // mojibake). 山田太郎 = U+5C71 U+7530 U+592A U+90CE. ~keep
    #[test]
    ...
    // ASCII stays a plain literal (PDFDocEncoding-compatible) — no UTF-16 bloat. ~keep
    #[test]
    ...
    fn test_wrapper_parent_name() {
        ...
        // but from_parent_config does ~keep
    ...
    fn test_wrapper_has_parent() {
        ...
        assert!(child_wrapper.has_parent());
    }
    ...
    fn test_build_parent_dict_basic() {
        ...
        assert!(matches!(dict.get("T"), Some(Object::String(b)) if b == b"address"));
    }
    ...
    fn test_build_parent_dict_with_field_type() {
        ...
        assert!(matches!(dict.get("FT"), Some(Object::Name(s)) if s == "Tx"));
    }
    ...
    fn test_build_parent_dict_with_button_type() {
        ...
        let wrapper = FormFieldWrapper::from_parent_config(&config);
        let dict = wrapper.build_parent_dict();
        assert!(matches!(dict.get("FT"), Some(Object::Name(s)) if s == "Btn"));
    }
    ...
    fn test_build_parent_dict_with_choice_type() {
        ...
        let wrapper = FormFieldWrapper::from_parent_config(&config);
        let dict = wrapper.build_parent_dict();
        assert!(matches!(dict.get("FT"), Some(Object::Name(s)) if s == "Ch"));
    }
    ...
    fn test_get_default_value_from_original() {
        ...
        // should return something (even though the current implementation has a limitation) ~keep
        let dv = wrapper.get_default_value();
        // Due to implementation limitation, it returns &FormFieldValue::None ~keep
        assert!(dv.is_some());
    }
