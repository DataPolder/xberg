...
use crate::writer::{ContentStreamBuilder, ObjectSerializer, hoist_appearance_streams};
...
impl Permissions {
    ...
    pub fn to_bits(&self) -> i32 {
        ...
        // Bits 7-8 (0-indexed: 6-7) and bits 13-32 (0-indexed: 12-31) must be 1 ~keep
        let mut bits: i32 = 0xFFFFF0C0u32 as i32;

        // Bit 3 (0-indexed: 2): Print ~keep
        if self.print {
            bits |= 1 << 2;
        }

        // Bit 4 (0-indexed: 3): Modify contents ~keep
        if self.modify {
            bits |= 1 << 3;
        }

        // Bit 5 (0-indexed: 4): Copy or extract text and graphics ~keep
        if self.copy {
            bits |= 1 << 4;
        }

        // Bit 6 (0-indexed: 5): Add or modify annotations ~keep
        if self.annotate {
            bits |= 1 << 5;
        }

        // Bit 9 (0-indexed: 8): Fill in form fields (R>=3) ~keep
        if self.fill_forms {
            bits |= 1 << 8;
        }

        // Bit 10 (0-indexed: 9): Extract text for accessibility (R>=3) ~keep
        if self.accessibility {
            bits |= 1 << 9;
        }

        // Bit 11 (0-indexed: 10): Assemble document (R>=3) ~keep
        if self.assemble {
            bits |= 1 << 10;
        }

        // Bit 12 (0-indexed: 11): Print high quality (R>=3) ~keep
...
///
/// ```ignore
/// use xberg_native_pdf::editor::{EncryptionConfig, EncryptionAlgorithm, Permissions};
///
...
pub struct DocumentEditor {
    ...
    /// (reachability alone is insufficient because the page is only
...
impl DocumentEditor {
    ...
    pub(crate) fn replace_source_bytes(&mut self, bytes: Vec<u8>) -> Result<()> {
        ...
        // Subsequent insert_modified/alloc calls will flip it back to true. ~keep
        Ok(())
    }
            ...
            if let Some(kid_ref) = kid.as_reference() {
                ...
                if let Some(kid_dict) = kid_obj.as_dict() {
                    ...
                    if type_name == "Page" {
                        ...
                        && let Some(sub_kids) = kid_dict.get("Kids").and_then(|k| k.as_array())
                    {
    ...
    pub fn extract_pages_to_bytes(&mut self, pages: &[usize]) -> Result<Vec<u8>> {
        ...
        // vector and aren't represented there. ~keep
        ...
        // list, so this staging only matters for the GC walk. ~keep
        let pages_ref = self
            .source
            .trailer()
            .as_dict()
        ...
        // Always restore — even on Err — so the document is observably unchanged. ~keep
    ...
    fn import_page_from_document(
        ...
        // source page tree (which causes cycles and imports unreachable objects) ~keep
        ...
        match obj {
            Object::Reference(obj_ref) => {
                ...
                if !visiting.insert(obj_ref.id) {
                    ...
                    // and return a reference; the object will be filled later ~keep
    ...
    fn write_incremental(&mut self, path: impl AsRef<Path>) -> Result<()> {
        ...
        // every copied stream plaintext. ~keep
        ...
        for (obj_id, offset, generation) in &xref_entries {
            write!(writer, "{} 1\n", obj_id)?;
        ...
        if let Ok(catalog) = self.source.catalog()
            && let Some(dict) = self.source.trailer().as_dict()
            && let Some(root_ref) = dict.get("Root")
        {
            write!(writer, "  /Root ")?;
    ...
    fn flush_form_fields_to_modified_objects(&mut self) -> Result<()> {
        ...
        for (obj_id, obj_gen, new_value, is_button) in &fields_to_flush {
            ...
            // For button fields (checkboxes/radios), also update /AS to match /V ~keep
            if *is_button {
    ...
    fn write_full_to_writer(
        ...
        // silently produces a structurally valid but unreadable PDF. ~keep
        ...
        write!(writer, "%PDF-{}.{}\n", major, minor)?;
        // Binary marker per spec (bytes > 127 to indicate binary content) ~keep
        ...
        let serialize_obj = |s: &ObjectSerializer,
         ...
         -> Vec<u8> {
            if let Some(h) = handler {
                s.serialize_indirect_encrypted(id, generation, obj, h)
            } else {
                s.serialize_indirect(id, generation, obj)
            }
        };
        ...
        // Write encryption dictionary if encrypting (must not be encrypted itself) ~keep
        let encrypt_obj_id = if let Some(ref enc_dict) = encrypt_dict {
            ...
            // Encryption dict is NOT encrypted ~keep
        ...
        if self.remove_acroform {
            ...
            // remain on non-flattened pages (ISO 32000-1 §12.7.2). ~keep
            if let Some(catalog_dict) = catalog_obj.as_dict()
                && let Some(rebuilt) = self.rebuild_partial_acroform(catalog_dict)?
            {
                let mut new_catalog = catalog_dict.clone();
        ...
        // Stores: (page_index, object_id, wrapper, is_root_field) ~keep
        ...
        if self.acroform_modified && !self.remove_acroform {
            ...
            // value. Also sets /NeedAppearances on the existing AcroForm. ~keep
            self.flush_form_fields_to_modified_objects()?;
            ...
            // AcroFormBuilder. ~keep
            if !self.remove_acroform
                && self.flatten_forms_pages.is_empty()
                && let Some(patched) = self.modified_objects.get(&catalog_ref.id)
            {
            ...
            // ids and their place in /Fields/Annots (updated in place above). ~keep
            ...
            // This ensures parents get IDs before children that reference them ~keep
            ...
            for mut wrapper in all_wrappers {
                ...
                if let Some(parent_name) = wrapper.parent_name()
                    && let Some(&parent_ref) = field_name_to_ref.get(parent_name)
                {
            ...
            if !all_form_field_data.is_empty() {
                ...
                // field would drop every pre-existing field from /Fields). ~keep
        ...
        if !embedded_files.is_empty() {
            ...
            if let Some(catalog_dict) = catalog_obj.as_dict() {
                ...
                // Sort by name for proper name tree ordering ~keep
            ...
            && let Some(pages_ref) = catalog_dict.get("Pages").and_then(|p| p.as_reference())
        {
                ...
                && let Some(kids) = pages_dict.get("Kids").and_then(|k| k.as_array())
            {
                ...
                // be keyed by the original source index, not the output position. ~keep
                ...
                for kid in kids {
                    if let Some(page_ref) = kid.as_reference() {
                        ...
                        // Merged pages (appended after source pages) use the loop counter. ~keep
                        ...
                        let has_erase_overlay = self.erase_regions.contains_key(&source_page_index);
                        ...
                        // overlay and the secret is physically gone. ~keep
                        ...
                        // ShowEmbeddedText path and never reach this registration). ~keep
                        ...
                        // original content) are lost when /Resources is rebuilt below. ~keep
                        if !overlay_font_ids.is_empty()
                            && let Some(page_dict) = final_page_obj.as_dict()
                        {
                            let mut new_dict = page_dict.clone();
                        ...
                        if let (Some((redactions, redact_overlay_id)), Some(page_dict)) =
                        ...
                        // Destructive redaction: REPLACE the
                        ...
                        // are removed (ISO 32000-1 §12.5.6.23). ~keep
                            ...
                            (&destructive_redaction, final_page_obj.as_dict())
                        {
                            ...
                            // need to infer it positionally from the array. ~keep
                            ...
                            && let Some(page_dict) = final_page_obj.as_dict()
                        {
                            ...
                            for annot_id in &new_annotation_ids {
                                annots_array.push(Object::Reference(ObjectRef::new(*annot_id, 0)));
                            }

                            for field_id in &new_form_field_ids {
                                annots_array.push(Object::Reference(ObjectRef::new(*field_id, 0)));
                            }

                            new_dict.insert("Annots".to_string(), Object::Array(annots_array));
                            final_page_obj = Object::Dictionary(new_dict);
                        }
                        ...
                        if let Some(page_dict) = page_obj.as_dict() {
                                ...
                                && self.modified_content.contains_key(&source_page_index)
                            {
                                ...
                                if has_image_mods {
                                    ...
                                    // page was destructively redacted (G6):
                                ...
                                page_dict.get("Resources").and_then(|r| r.as_reference())
                            {
                                ...
                                if !new_xobject_refs.is_empty()
                                    && let Some(res_dict) = resources_obj.as_dict()
                                {
                                ...
                                tracing::warn!(
                                    page_index,
                                    count = new_xobject_refs.len(),
                                    "page has inline Resources dict; new image XObject(s) could not be added to \
                                     Resources/XObject"
                                );
                            }
                        ...
                        if let Some(additions_id) = overlay_additions_id
                            && let Some(added) = self.overlay_additions.get(&source_page_index)
                        {
                        ...
                        if let Some((ref appearances, overlay_id, ref xobj_ids)) = flatten_data {
                                ...
                                xobj_ids.iter().zip(appearances.iter())
                            {
                                ...
                                if let Some(ref resources) = appearance.resources {
                                    form_dict.insert("Resources".to_string(), resources.clone());
                                }
                        ...
                        if let Some((ref redactions, redact_overlay_id)) = redaction_data {
                            let overlay_content = self.generate_redaction_overlay(redactions);
        ...
        // Rebuild written_ids for O(1) dedup lookups in merged page loop ~keep
        written_ids.clear();
        ...
        for (page_data, &page_id) in self.merged_pages.iter().zip(merged_page_ids.iter()) {
            let final_page_obj = if let Some(catalog_dict) = catalog_obj.as_dict() {
                if let Some(pages_ref) = catalog_dict.get("Pages").and_then(|p| p.as_reference()) {
                    match page_data.page_object.clone() {
        ...
        // These don't belong to any specific page, so write them after page processing ~keep
        for (_, field_id, wrapper, _) in &all_form_field_data {
        ...
        // without updating `written_ids`. ~keep
        written_ids.clear();
        written_ids.extend(xref_entries.iter().map(|(id, _, _, _)| *id));
        ...
        for obj_id in all_source_ids {
            ...
            // G6: never emit the original content stream of a
            // destructively-redacted page (it holds the secret bytes),
            // regardless of GC reachability. ~keep
            if self.redacted_orphan_ids.contains(&obj_id) {
                continue;
            }
            if let Some(ref reachable) = reachable_ids
                && !reachable.contains(&obj_id)
            {
                tracing::trace!(object_id = obj_id, "GC dropping unreachable object");
                continue;
            }
            // Prefer a staged modification over the original source object. ~keep
            let loaded = if let Some(m) = self.modified_objects.get(&obj_id) {
                Ok(m.clone())
            } else {
                self.load_source_object(ObjectRef {
                    id: obj_id,
                    generation: 0,
                })
            };
        ...
        // original source xref (e.g. XMP streams allocated by the PDF/A converter). ~keep
        {
        ...
        let mut entry_map: HashMap<u32, (u64, u16, bool)> = xref_entries
            .into_iter()
            .map(|(id, off, generation, used)| (id, (off, generation, used)))
            .collect();

        for id in 0..=max_id {
            if let Some((offset, generation, in_use)) = entry_map.get(&id) {
                if *in_use {
                    write!(writer, "{:010} {:05} n \n", offset, generation)?;
                } else {
                ...
                // Free entry pointing to object 0 ~keep
        ...
        if let Some(info_ref) = info_ref {
            write!(writer, "  /Info {} {} R\n", info_ref.id, info_ref.generation)?;
        }
    ...
    pub fn get_page(&mut self, page_index: usize) -> Result<crate::editor::dom::PdfPage> {
        ...
        let content = if let Some(structure) = self.get_page_content(page_index)? {
            ...
            match HierarchicalExtractor::extract_page(&self.source, page_index)? {
                ...
                None => StructureElement {
                    structure_type: "Document".to_string(),
                    bbox: crate::geometry::Rect::new(0.0, 0.0, page_info.width, page_info.height),
                    children: Vec::new(),
    ...
    pub fn save_page(&mut self, page: crate::editor::dom::PdfPage) -> Result<()> {
        ...
        if page.is_loaded_from_source() {
            ...
            // `save_page()` continue to see the expected true value. ~keep
            self.is_modified = true;
            ...
            // real content stream instead of silently dropping the edit. ~keep
    ...
    pub fn get_page_rotation(&mut self, index: usize) -> Result<i32> {
        ...
        if let Some(props) = self.modified_page_props.get(&source_index)
            && let Some(rotation) = props.rotation
        {
            return Ok(rotation);
        }
    ...
    pub fn get_page_media_box(&mut self, index: usize) -> Result<[f32; 4]> {
        let source_index = self.output_to_source_index(index);
        if let Some(props) = self.modified_page_props.get(&source_index)
            && let Some(media_box) = props.media_box
        {
            return Ok(media_box);
        }
    ...
    pub fn get_page_crop_box(&mut self, index: usize) -> Result<Option<[f32; 4]>> {
        let source_index = self.output_to_source_index(index);
        if let Some(props) = self.modified_page_props.get(&source_index)
            && let Some(crop_box) = props.crop_box
        {
    ...
    fn rebuild_partial_acroform(
        ...
        // Leave the AcroForm as-is and warn the caller (ISO 32000-1 §12.7.8). ~keep
        ...
        // (per-index get_page_ref is O(n), looping it is O(n²)). ~keep
        ...
        for field_entry in &fields_array {
            if let Object::Reference(field_ref) = field_entry
                && self.field_has_surviving_widgets(field_ref, &flattened, &page_ref_to_index)?
            {
                surviving.push(Object::Reference(*field_ref));
            }
        }

        // Preserve all original top-level AcroForm keys; replace only /Fields. ~keep
    ...
    fn field_has_surviving_widgets(
        ...
        let field_obj = match self.source.load_object(*field_ref) {
            Ok(o) => o,
            Err(_) => return Ok(true), // can't load → keep to be safe ~keep
        };
    ...
    pub fn get_form_fields(&mut self) -> Result<Vec<FormFieldWrapper>> {
        ...
        // one tree walk (per-index get_page_ref is O(n), looping is O(n²)). ~keep
        ...
        for field in source_fields {
            ...
            if let Some(wrapper) = self.modified_form_fields.get(&full_name) {
                ...
                let page_index = field
                    ...
                    .and_then(|dict| {
                        // Check /P on the field dict first (merged field+widget) ~keep
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::editor::{DocumentEditor, ParentFieldConfig};
    ///
    /// let mut editor = DocumentEditor::open("document.pdf")?;
    ///
    ...
    pub fn add_parent_field(
        ...
        if let Some(ref parent_name) = config.parent_name
            && !self.has_form_field(parent_name)?
        {
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::editor::{DocumentEditor, FormFieldValue};
    ///
    /// let mut editor = DocumentEditor::open("form.pdf")?;
    ///
    ...
    fn get_widget_appearances(&mut self, page: usize) -> Result<Vec<AnnotationAppearance>> {
        ...
        for annotation in annotations {
            ...
            // shows what was set, instead of baking the blank placeholder. ~keep
            if let Some(text) = self.modified_widget_text(&annotation) {
                ...
                // return None here and fall through to the plain /DA path. ~keep
    ...
    fn modified_widget_text(&self, annotation: &crate::annotations::Annotation) -> Option<String> {
        ...
        // name first, then by trailing segment for hierarchical fields. ~keep
    ...
    fn is_self_contained_simple_font(font: &Object) -> bool {
        ...
        // A FontDescriptor implies an embedded program / extra object graph. ~keep
    ...
    fn embed_fallback_text_appearance(
        ...
        use crate::writer::{EmbeddedFont, build_embedded_font_objects};
        ...
        if let Object::Dictionary(res) = self.build_text_appearance_resources(&da_font)
            && let Some(Object::Dictionary(f)) = res.get("Font")
        {
            font_res = f.clone();
        }
        ...
        for run in &runs {
            if let Some(kind) = run.fallback
                && let std::collections::btree_map::Entry::Vacant(e) = fonts.entry(kind)
            {
        ...
        // Emit the appearance content per ISO 32000-1 §12.7.3.3. ~keep
    ...
    /// (ISO 32000-1:2008 §12.5.6.23). `fill` is the overlay colour
            ...
            Object::Array(arr) => {
                ...
                for item in arr {
                    if let Object::Reference(r) = item
                        && let Ok(s) = self.source.load_object(r)?.decode_stream_data()
                    {
    ...
    fn build_page_font_metrics(
        ...
        // (fail closed, never under-redact). ~keep
        let page_ref = self.source.get_page_ref(source_page)?;
        ...
        while let Some(obj) = node {
            guard += 1;
            if guard > 64 {
                break; // cyclic /Parent — bail (fail closed) ~keep
            }
    ...
    fn apply_redactions_destructive_for_page(
        ...
        use crate::redaction::{RedactionRegion, RegionSet, redact_content_stream};
        ...
        // an orphaned, GC-missed object. ~keep
        if let Ok(page_ref) = self.source.get_page_ref(src)
            && let Ok(page_obj) = self.source.load_object(page_ref)
            && let Some(d) = page_obj.as_dict()
        {
    ...
    pub fn sanitize_document(
        ...
        // post-scrub reachability set reflects them. ~keep
        self.modified_objects
        ...
        // annotation — must not be dropped). ~keep
        ...
        while let Some(id) = queue.pop_front() {
            ...
            if let Ok(obj) = self.source.load_object(ObjectRef { id, generation: 0 }) {
        ...
        // (same defense-in-depth as redacted content streams, G6). ~keep
        if opts.scrub_metadata
            && let Some(id) = info_root
            && to_drop.insert(id)
            && let Ok(obj) = self.source.load_object(ObjectRef { id, generation: 0 })
        {
            account(&obj, &mut bytes_removed);
        }
        ...
        for op in operators {
            match op {
                ...
                crate::content::operators::Operator::Do { ref name } => {
                    ...
                    // would check the XObject's Subtype ~keep
                    let matrix = current_ctm;

                    // Extract position and size from matrix
                    // Standard image matrix: [width, 0, 0, height, x, y] ~keep
                    let x = matrix[4];
                    let y = matrix[5];
    ...
    fn rewrite_content_stream_with_image_mods(
        ...
        while i < operators.len() {
            ...
            // We need to find cm operators that precede Do operators ~keep
            match op {
                crate::content::operators::Operator::Cm { a, b, c, d, e, f } => {
                    let mut j = i + 1;
                    ...
                    if let Some(name) = found_do
                        && let Some(modification) = modifications.get(&name)
                    {
    ...
    /// of truth (DRY, T1). Binary strings are emitted as
...
impl EditableDocument for DocumentEditor {
    fn get_info(&mut self) -> Result<DocumentInfo> {
        ...
        if let Some(trailer_dict) = trailer.as_dict()
            && let Some(info_ref) = trailer_dict.get("Info").and_then(|i| i.as_reference())
        {
...
mod tests {
    ...
    fn test_document_info_creator_producer() {
        let info = DocumentInfo::new()
            .creator("xberg-native-pdf")
            .producer("Rust PDF Library");
        assert_eq!(info.creator, Some("xberg-native-pdf".to_string()));
    ...
    fn create_test_editor() -> DocumentEditor {
        ...
        let tmp = dir.path().join("native_pdf_test.pdf");
    ...
    fn test_source_path() {
        let pdf_bytes = minimal_pdf_bytes();
        let dir = tempfile::tempdir().expect("create temp dir");
        let tmp = dir.path().join("native_pdf_test_path.pdf");
        std::fs::write(&tmp, &pdf_bytes).unwrap();
        let editor = DocumentEditor::open(&tmp).unwrap();
        assert!(editor.source_path().contains("native_pdf_test_path.pdf"));
        let _ = std::fs::remove_file(&tmp);
    }
    ...
    fn test_open_nonexistent_file() {
        let result = DocumentEditor::open("/tmp/nonexistent_native_pdf_test.pdf");
        assert!(result.is_err());
    }
    ...
    fn create_multi_page_editor(n: usize) -> DocumentEditor {
        ...
        let dir = tempfile::tempdir().expect("create temp dir");
        let tmp = dir.path().join("native_pdf_multipage_test.pdf");
    ...
    fn test_extract_pages_works() {
        let mut editor = create_test_editor();
        let dir = tempfile::tempdir().expect("create temp dir");
        let out = dir.path().join("native_pdf_extract_test.pdf");
    ...
    fn test_extract_pages_out_of_range() {
        let mut editor = create_test_editor();
        let dir = tempfile::tempdir().expect("create temp dir");
        let out = dir.path().join("native_pdf_extract_oob.pdf");
    ...
    fn test_crop_margins() {
        ...
        assert!((crop[0] - 72.0).abs() < 0.01);
        assert!((crop[1] - 72.0).abs() < 0.01);
        assert!((crop[2] - 540.0).abs() < 0.01);
        assert!((crop[3] - 720.0).abs() < 0.01);
    }
    ...
    fn create_test_editor_with_text() -> DocumentEditor {
        ...
        let dir = tempfile::tempdir().expect("create temp dir");
        let tmp = dir.path().join("native_pdf_redact_addtext_test.pdf");
        std::fs::write(&tmp, &pdf_bytes).unwrap();
        let editor = DocumentEditor::open(&tmp).unwrap();
        let _ = std::fs::remove_file(&tmp);
        editor
    ...
    fn destructive_redaction_preserves_add_text_overlay() {
        ...
        let dir = tempfile::tempdir().expect("create temp dir");
        let tmp = dir.path().join("native_pdf_redact_addtext_verify.pdf");
    ...
    fn multiple_add_text_calls_on_same_page_all_survive() {
        ...
        let bytes = editor.save_to_bytes().unwrap();
        let dir = tempfile::tempdir().expect("create temp dir");
        let tmp = dir.path().join("native_pdf_multi_addtext_verify.pdf");
        std::fs::write(&tmp, &bytes).unwrap();
        let mut verify = DocumentEditor::open(&tmp).unwrap();
        let _ = std::fs::remove_file(&tmp);
    ...
    fn open_editor_from_bytes(bytes: &[u8], tag: &str) -> DocumentEditor {
        let dir = tempfile::tempdir().expect("create temp dir");
        let tmp = dir.path().join(format!("native_pdf_{tag}.pdf"));
        std::fs::write(&tmp, bytes).unwrap();
        let editor = DocumentEditor::open(&tmp).unwrap();
        let _ = std::fs::remove_file(&tmp);
        editor
