...
//! use xberg_native_pdf::writer::LinkAnnotation;
...
impl PdfTable {
    ...
    pub fn set_cell_text(&mut self, row: usize, col: usize, text: impl Into<String>) -> bool {
        if let Some(row_content) = self.content.rows.get_mut(row)
            && let Some(cell) = row_content.cells.get_mut(col)
        {
            cell.text = text.into();
            return true;
        }
...
impl AnnotationWrapper {
    ...
    pub fn color(&self) -> Option<(f32, f32, f32)> {
        if let Some(ref original) = self.original
            && let Some(ref color) = original.color
            && color.len() >= 3
        {
...
impl PdfPage {
    ...
    fn record_original_text_edit(&mut self, path: &ElementPath) {
        ...
        if !text.text.is_empty() {
            ...
            // existing element and silently strand any ID a caller is holding. ~keep
            let overlay = text;
    ...
    pub fn get_parent(&self, id: ElementId) -> Option<PdfElement> {
        ...
        // for simplicity, as root is a synthetic container) ~keep
        if path.path.len() <= 1 {
            return None;
        }
    ...
    pub fn remove_element(&mut self, id: ElementId) -> bool {
            ...
            && path.path.len() == 1
        {
            ...
            if idx < self.root.children.len() {
                ...
                // the remaining original elements are still recognized as such. ~keep
                ...
                // Rebuild element map since indices have shifted ~keep
