...
impl ObjectSerializer {
    ...
    ///
    /// Format: `{id} {gen} obj\n{object}\nendobj\n`
    pub fn serialize_indirect(&self, id: u32, generation: u16, obj: &Object) -> Vec<u8> {
        let mut buf = Vec::new();
        writeln!(buf, "{} {} obj", id, generation).unwrap();
        self.write_object(&mut buf, obj).unwrap();
        write!(buf, "\nendobj\n").unwrap();
        buf
    }
    ...
    pub fn serialize_indirect_encrypted(
        ...
        let mut buf = Vec::new();
        writeln!(buf, "{} {} obj", id, generation).unwrap();
        self.write_object_encrypted(&mut buf, obj, id, generation, handler)
            .unwrap();
        write!(buf, "\nendobj\n").unwrap();
        buf
    }
    ...
    fn write_stream_encrypted<W: Write>(
        ...
        // Update dictionary with encrypted length ~keep
    ...
    fn write_real<W: Write>(&self, w: &mut W, value: f64) -> std::io::Result<()> {
        ...
        // Remove trailing zeros for compact output ~keep
        if value.fract() == 0.0 {
...
impl ObjectSerializer {
    ...
    pub fn reference(id: u32, generation: u16) -> Object {
        Object::Reference(ObjectRef::new(id, generation))
    }
    ...
    if let Some(Object::Dictionary(ap)) = annot_dict.get_mut("AP") {
        for key in ["N", "D", "R"] {
            match ap.get_mut(key) {
                ...
                // checkbox's `/On` and `/Off`); hoist each state's stream. ~keep
...
mod tests {
    ...
    fn test_hoist_appearance_stream_replaces_inline_n_with_reference() {
        ...
        // separate writing -- a stream is illegal as a direct dict value. ~keep
