...
//!
//! ```ignore
//! use xberg_native_pdf::writer::{LineAnnotation, ShapeAnnotation, PolygonAnnotation};
//! use xberg_native_pdf::geometry::Rect;
//! use xberg_native_pdf::annotation_types::LineEndingStyle;
//!
...
impl PolygonAnnotation {
    ...
    pub fn build(&self, _page_refs: &[ObjectRef]) -> HashMap<String, Object> {
        ...
        if let Some((start, end)) = &self.line_endings
            && self.polygon_type == PolygonType::PolyLine
        {
            dict.insert(
                "LE".to_string(),
                Object::Array(vec![
    ...
    fn test_polygon_build_vertices_array() {
        ...
        match dict.get("Vertices") {
            Some(Object::Array(arr)) => {
                // 3 vertices x 2 coords = 6 values ~keep
                assert_eq!(arr.len(), 6);
