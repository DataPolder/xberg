...
impl XmpWriter {
    ...
    pub fn default_metadata() -> Self {
        ...
        metadata.xmp_creator_tool = Some("xberg-native-pdf".to_string());
        metadata.pdf_producer = Some(format!("xberg-native-pdf {}", env!("CARGO_PKG_VERSION")));
        Self { metadata }
    }
    ...
    fn to_xml(&self) -> String {
        ...
        // randomized order and the packet's bytes differ between runs. ~keep
        ...
        // 2KB of padding is standard ~keep
        for _ in 0..40 {
            xml.push_str("                                                  \n");
        }
...
mod tests {
    ...
    fn test_xmp_writer_pdf_properties() {
        let writer = XmpWriter::new(XmpMetadata::new())
            .producer("native_pdf 0.3.0")
            .keywords("test, pdf, metadata")
            .pdf_version("1.7");

        let xml = writer.build();

        assert!(xml.contains("<pdf:Producer>native_pdf 0.3.0</pdf:Producer>"));
