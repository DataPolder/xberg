...
//!
//! ```ignore
//! use xberg_native_pdf::writer::{ThreeDAnnotation, ThreeDStream, ThreeDView};
...
// Allow uppercase acronyms for industry-standard format names (U3D, PRC, CAD) ~keep
...
impl ThreeDCamera {
    ...
    pub fn calculate_matrix(&self) -> Vec<f64> {
        ...
        // Camera-to-world matrix (3x4) ~keep
        vec![
            xx_r as f64,
            xy_r as f64,
            xz_r as f64,
...
impl ThreeDView {
    ...
    pub fn build(&self) -> HashMap<String, Object> {
        ...
        // XN - external name (required) ~keep
        ...
        // IN - internal name (same as external) ~keep
        if let Some(ref name) = self.name {
            dict.insert("IN".to_string(), Object::text_string(name));
        }

        // C2W - camera-to-world transformation matrix ~keep
...
impl ThreeDAnnotation {
    ...
    pub fn build(&self, _page_refs: &[ObjectRef]) -> HashMap<String, Object> {
        ...
        // For now, include the view inline ~keep
        dict.insert("3DV".to_string(), Object::Dictionary(self.default_view.build()));

        // Note: 3DD (3D stream reference) must be added by the caller ~keep
...
mod tests {
    ...
    fn test_threed_annotation_build() {
        ...
        assert!(dict.contains_key("3DA"));
        assert!(dict.contains_key("3DV"));
    }
}
