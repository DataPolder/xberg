...
//!
//! ```ignore
//! use xberg_native_pdf::writer::{ScreenAnnotation, MediaClip, MediaRendition};
...
impl MediaClip {
    ...
    pub fn build(&self, file_spec_ref: Option<ObjectRef>) -> HashMap<String, Object> {
        ...
        dict.insert("S".to_string(), Object::Name("MCD".to_string()));

        // N - name/filename ~keep
        dict.insert("N".to_string(), Object::text_string(&self.filename));

        // D - file specification (if provided) ~keep
        if let Some(ref_obj) = file_spec_ref {
            dict.insert("D".to_string(), Object::Reference(ref_obj));
        }

        // CT - content type (MIME) ~keep
        if let Some(ref ct) = self.content_type {
            dict.insert("CT".to_string(), Object::text_string(ct));
        }

        // Alt - alternate text (array of language/text pairs) ~keep
        if let Some(ref alt) = self.alt_text {
            dict.insert(
                "Alt".to_string(),
                Object::Array(vec![Object::String(Vec::new()), Object::text_string(alt)]),
            );
        }

        // P - media permissions ~keep
...
impl WindowType {
    ...
    pub fn pdf_value(&self) -> i64 {
        match self {
            WindowType::Annotation => 3,
            WindowType::Floating => 0,
            WindowType::FullScreen => 1,
            WindowType::Hidden => 2,
        }
    }
}
...
impl MediaPlayParams {
    ...
    pub fn build(&self) -> HashMap<String, Object> {
        ...
        // RC - repeat count (0 = infinite) ~keep
        if self.repeat_count != 1 {
            dict.insert("RC".to_string(), Object::Real(self.repeat_count as f64));
        }

        // BE - best effort parameters including controls visibility ~keep
        ...
        // F - floating window parameters ~keep
        if self.window_type != WindowType::Annotation {
            let mut fw = HashMap::new();
            // W - window type (0=floating, 1=fullscreen, 2=hidden, 3=annotation) ~keep
...
impl MediaRendition {
    ...
    pub fn build(&self, clip_ref: Option<ObjectRef>) -> HashMap<String, Object> {
        ...
        dict.insert("S".to_string(), Object::Name("MR".to_string()));
...
impl ScreenAnnotation {
    ...
    pub fn build(&self, _page_refs: &[ObjectRef]) -> HashMap<String, Object> {
        ...
        // MK - appearance characteristics (border) ~keep
        let mut mk = HashMap::new();
        mk.insert(
            "BG".to_string(),
            Object::Array(vec![Object::Real(0.9), Object::Real(0.9), Object::Real(0.9)]),
        );
        dict.insert("MK".to_string(), Object::Dictionary(mk));

        // A - Rendition action
        // Note: The actual rendition reference (R) must be added by the caller ~keep
        ...
        // AN - annotation reference (this annotation) - must be added by caller ~keep
...
mod tests {
    ...
    fn test_media_play_params_build() {
        let params = MediaPlayParams::new().with_volume(0.7).with_repeat(0);

        let dict = params.build();

        assert!(dict.contains_key("V"));
        assert!(dict.contains_key("RC"));
    }
