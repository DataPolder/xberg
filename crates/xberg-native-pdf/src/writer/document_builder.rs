...
/// Marker style for `numbered_list`.
...
/// Emitted as the `/Tabs` entry on the page dict.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TabOrder {
...
pub struct FluentPageBuilder<'a> {
    ...
    /// applied to `TextContent.matrix` on emission. Currently text
    /// only — see inline comment in the transforms section for the
    /// Path / Image / Table follow-up.
    current_matrix: Option<[f32; 6]>,
    /// Deferred `/Tabs` value for the current page, set by
    /// [`Self::tab_order`] and applied in [`Self::done`].
...
impl<'a> FluentPageBuilder<'a> {
    ...
    ///
    /// The streaming `Table` surface uses this to decide
    ...
    /// `streaming_table` or to right-align custom labels. For
    ...
    pub fn text_in_rect(mut self, rect: Rect, text: &str, align: TextAlign) -> Self {
        ...
        // per-line; the default matches the behaviour of `.text()`. ~keep
    ...
    /// (not yet implemented — filed as an enhancement).
    ...
    /// `language` is cosmetic — it doesn't syntax-highlight (syntax
    /// highlighting is deferred) but is recorded for future
    /// accessibility tagging (Bundle F).
    ///
    /// After this call the cursor advances past the block.
    ///
    /// ```no_run
    /// # use xberg_native_pdf::writer::DocumentBuilder;
    /// # let mut doc = DocumentBuilder::new();
    ...
    pub fn code_block(mut self, language: &str, source: &str) -> Self {
        ...
        for (line_idx, (line, _)) in all_lines.iter().enumerate() {
            ...
            // the prior one (double-advance). ~keep
            self.cursor_y += line_height;
        }
    ...
    pub fn paragraph(mut self, text: &str) -> Self {
        ...
        for (line_text, line_width) in lines {
            ...
            // margin; otherwise content silently writes off the page (invisible). ~keep
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::writer::StampType;
    ///
    /// builder.page(PageSize::Letter)
    ///     .at(72.0, 720.0)
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::writer::{LinkAnnotation, Annotation};
    ...
    ///
    /// Wires the existing `ListBoxWidget` through the fluent surface.
    pub fn list_box(
        self,
        name: impl Into<String>,
        x: f32,
    ...
    ///
    /// This is the buffered table surface.
    ...
    /// are measured against the table default — good enough for now.
    /// Tracked as issue #400 (mixed-font precise metrics).
    pub fn table(mut self, table: Table) -> Self {
        let page_width = self.builder.pages[self.page_index].width;
        let content_width = page_width - 2.0 * 72.0;
        ...
        for (i, mut elem) in elements.into_iter().enumerate() {
            ...
            // sequence so subsequent builder calls don't alias orders. ~keep
            match &mut elem {
    ...
    /// coloured / variable-thickness cell borders.
    ...
    // Table-as-a-whole matrix remains unimplemented. Extended from
    // text-only to cover paths + images as a follow-up.
    // ─────────────────────────────────────────────────────────────────── ~keep
        ...
        F: FnOnce(Self) -> Self,
    {
        ...
        let (c, s) = (r.cos(), r.sin());
        // PDF matrix row order: [a b c d e f] = [cos sin -sin cos 0 0] ~keep
    ...
    fn image_with_options(
        ...
        // etc. scopes render under that matrix. ~keep
    ...
    // `push_stroked_fill` applies both to a `PathContent`. ~keep
    ...
    pub fn arc(self, cx: f32, cy: f32, radius: f32, start_angle: f32, end_angle: f32, stroke: LineStyle) -> Self {
        ...
        while a < b {
            ...
            // origin: (4/3) * tan(sweep/4) × radius. ~keep
    ...
    fn push_shaped_path(
        ...
        // `.scaled()` / `.translated()` scopes render under that matrix. ~keep
        path.matrix = self.current_matrix;
        let page = &mut self.builder.pages[self.page_index];
        page.elements.push(ContentElement::Path(path));
        self
...
///
/// ```ignore
/// use xberg_native_pdf::writer::{DocumentBuilder, PageSize, DocumentMetadata};
///
...
impl DocumentBuilder {
    ...
    pub fn insert_toc(self, insert_at: usize, title: impl Into<String>) -> Self {
        ...
        // so hierarchies render with left-indent per depth. ~keep
        ...
        // get an exclusive ref to render into it. ~keep
        ...
        // append at the end (no panic). ~keep
        ...
        // instead borrow a fresh builder pointing at `insert_pos`. ~keep
        {
            ...
            for (depth, t, target_page) in entries {
                ...
                // Right-aligned page number (1-based for user display). ~keep
    ...
    /// [`crate::writer::PageLabelsBuilder`].
    ///
    /// ```no_run
    /// # use xberg_native_pdf::writer::{DocumentBuilder, PageLabelsBuilder};
    ...
    /// ```no_run
    /// # use xberg_native_pdf::writer::DocumentBuilder;
    /// # use xberg_native_pdf::writer::outline_builder::OutlineItem;
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::writer::{DocumentBuilder, EmbeddedFont};
    ///
    ...
    pub fn build(self) -> Result<Vec<u8>> {
        ...
        for (idx, page_data) in self.pages.iter().enumerate() {
            ...
            if !page_data.pending_footnotes.is_empty() {
                ...
                // Separator line — marked as Layout artifact. ~keep
...
fn compose_affine(outer: [f32; 6], inner: [f32; 6]) -> [f32; 6] {
    // Row-major matrix multiply: (inner * outer)_{ij} = Σ_k inner_{ik} * outer_{kj} ~keep
...
mod tests {
    ...
    fn test_watermark_appearance_is_indirect_not_inline() {
        ...
        // the annotation dict (which is invalid PDF and renders to nothing). ~keep
        let mut builder = DocumentBuilder::new();
    ...
    // Minimal check for `/N <digits> 0 R` without pulling in a regex crate. ~keep
        ...
        doc.letter_page()
            ...
            .table(
                RenderTable::new(vec![vec![TableCell::text("a"), TableCell::text("b")]])
                    .with_column_widths(vec![ColumnWidth::Fixed(100.0), ColumnWidth::Fixed(100.0)]),
            )
    ...
    fn test_transforms_extend_to_path_and_image() {
        ...
        // must wrap them in `q ... cm ... Q` brackets. ~keep
        let mut doc = DocumentBuilder::new();
        doc.letter_page()
            .rotated(30.0, |p| {
    ...
    fn test_bookmark_and_tree_emit_outlines_in_catalog() {
        ...
        // a hex form `<4974726F>` depending on content. ~keep
