...
//!
//! Paired with `font_discovery` — the two
...
pub fn shape(text: &str, face_bytes: &[u8], direction: Direction) -> Option<ShapedRun> {
    ...
    // the shaper to select the right GSUB/GPOS rules. ~keep
    ...
    for (info, pos) in infos.iter().zip(positions.iter()) {
        ...
        // HarfBuzz's documented behaviour for the output buffer). ~keep
...
mod tests {
    ...
    const DEJAVU: &[u8] = include_bytes!("../fonts/assets/DejaVuSans.ttf");
    ...
    fn shape_rtl_arabic() {
        ...
        // in visual order with non-zero advances. ~keep
