...
//! the API caller.
//!
//! `load_system_fonts` walks the host font directories, so on wasm32
//! (no filesystem) it simply yields an empty database rather than
//! failing.

use crate::vendor::fontdb::{self, Database, Family, ID};
        ...
        let resolved_meta: Option<(String, PathBuf, String)> = {
            ...
            for &family in families {
                ...
                let fontdb_family = match family_norm.to_ascii_lowercase().as_str() {
                    ...
                    // practice when no system UI metadata is available. ~keep
...
fn face_metadata(db: &Database, id: ID, matched_family: &str) -> Result<(PathBuf, String), FontResolveError> {
    ...
    let path = match &face.source {
        ...
        // as one `File(path)` entry per face (differing only in `index`),
        // so no separate collection-handling arm is needed here. Upstream
        // fontdb also has a `SharedFile` source, but it's only ever
        // constructed by an unsafe, memmap-backed sharing API this crate
        // never calls — dropped when fontdb was vendored, see
        // `crate::vendor::fontdb`. ~keep
        ...
        // caller can choose a different family. ~keep
    ...
    let postscript_family = face
        .families
        .iter()
        .find(|(_, lang)| lang.as_deref().is_some_and(|l| l.starts_with("en")))
...
mod tests {
    ...
    fn resolves_generic_sans_serif() {
        ...
        // NoMatch but only when fontdb literally found nothing. ~keep
        match resolved {
            Ok(r) => {
