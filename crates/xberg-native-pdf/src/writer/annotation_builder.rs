...
//!
//! ```ignore
//! use xberg_native_pdf::writer::{LinkAnnotation, AnnotationBuilder, Annotation};
...
impl AnnotationBuilder {
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::writer::{AnnotationBuilder, LinkAnnotation};
    /// use xberg_native_pdf::geometry::Rect;
    ///
    /// let mut builder = AnnotationBuilder::new();
...
mod tests {
    ...
    fn test_link_annotation_build_goto_out_of_range() {
        ...
        let page_refs = vec![ObjectRef::new(10, 0)];
        let dict = link.build(&page_refs);
        // Should not contain Dest since page 99 doesn't exist ~keep
        assert!(!dict.contains_key("Dest"));
    }
