...
//!
//! ```ignore
//! use xberg_native_pdf::writer::{OutlineBuilder, OutlineItem};
//!
...
mod tests {
    ...
    fn test_outline_build() {
        ...
        assert!(result.objects.contains_key(&100));
        assert!(result.objects.contains_key(&101));
        assert!(result.objects.contains_key(&102));
    }
