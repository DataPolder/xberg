...
impl Default for PdfWriterConfig {
    fn default() -> Self {
        Self {
            ...
            subject: None,
            keywords: None,
            creator: Some("xberg-native-pdf".to_string()),
            compress: false,
...
impl<'a> PageBuilder<'a> {
    ...
    pub fn add_embedded_text(
        ...
        // often during early development. ~keep
        let glyph_ids = self
            .writer
            .embedded_fonts
    ...
    pub fn add_shaped_embedded_text(
        ...
        // is released before we call `encode_shaped_run` (&mut self). ~keep
        ...
        // shaper's cluster field so the ToUnicode CMap round-trips. ~keep
    ...
    pub fn add_element(&mut self, element: &ContentElement) -> &mut Self {
        if let ContentElement::Text(t) = element {
            ...
            // immutable ref survive the mutable call. ~keep
    ...
    /// only meaningful once Bundle F lands).
    ...
    pub fn add_text_field(&mut self, field: TextFieldWidget) -> &mut Self {
        let page_ref = ObjectRef::new(0, 0); // Will be resolved during finish() ~keep
    ...
    pub fn add_radio_group(&mut self, group: RadioButtonGroup) -> &mut Self {
        ...
        // The parent is a non-widget field that groups all radio buttons ~keep
        let parent_entry = FormFieldEntry {
            widget_dict: HashMap::new(),
...
impl PdfWriter {
    ...
    pub fn finish(mut self) -> Result<Vec<u8>> {
        ...
        // Binary marker (recommended for binary content) ~keep
        ...
        // identical so the reference resolves. ~keep
        ...
        // `build_with_remappers` below. FONT-3b. ~keep
        ...
        // First collect annotation counts to avoid borrow conflict ~keep
        ...
        // produce the same hash for the same logical image — fixes #443. ~keep
        ...
        for pending in &pending_per_page {
            ...
            for p in pending {
                ...
                // here regardless of how the caller originally stored the pixels. ~keep
        ...
        for (i, page_data) in self.pages.iter().enumerate() {
            ...
            let (content_bytes, is_compressed) = if self.config.compress {
                match compress_data(&raw_content) {
                    ...
                    Err(_) => (raw_content, false), // Fall back to uncompressed on error ~keep
                }
            } else {
                (raw_content, false)
            };
            ...
            for (j, field_entry) in page_data.form_fields.iter().enumerate() {
                ...
                // Merge widget entries into field dict (merged field/widget) ~keep
            ...
            // the ParentTree number-tree uses to look up its StructElems. ~keep
        ...
        let outline_ref = if let Some(outline) = self.outline.take() {
            ...
            let page_object_refs: Vec<ObjectRef> = page_objects
                ...
                .collect();
            match outline.build(&page_object_refs, self.next_obj_id) {
                Some(result) => {
        ...
        // the serialisation pass below can emit them in order. ~keep
        ...
        let struct_tree_root_id: Option<u32> = if self.config.tagged {
            ...
            fn emit_struct_elems(
                ...
                *next_id += 1;
                ...
                // Recurse into children first so we know their refs for /K ~keep
                ...
                // /K: either array of MCIDs + child refs, or just the MCID ~keep
            ...
            // ParentTree entries: page_index → [StructElem refs on that page] ~keep
            ...
            // ParentTree number-tree (flat /Nums array form) ~keep
            ...
            // the next key that would be assigned (i.e. the page count). ~keep
        ...
        // streams are appended so they get xref entries below. ~keep
        ...
        if let Some(acroform_id) = acroform_id
            && let Some(acroform_obj) = self.objects.get(&acroform_id)
        {
            xref_offsets.push((acroform_id, output.len()));
            output.extend_from_slice(&serializer.serialize_indirect(acroform_id, 0, acroform_obj));
        }
        ...
        // Emit in insertion order (struct_tree_obj_ids preserves this). ~keep
        ...
        // ISO 14289-1 §6.7.11: XMP metadata stream (tagged PDF only). ~keep
        if let Some(xmp_id) = xmp_metadata_id
            && let Some(obj) = self.objects.get(&xmp_id)
        {
        ...
        // Sort xref entries by object ID ~keep
...
pub(crate) fn image_content_to_xobject_stream(
    ...
    // buffer but labelled it as something else. ~keep
    let raw = &image.data;

    // PNG magic: \x89 P N G \r \n \x1a \n ~keep
    if raw.len() >= 8
        && raw[..8] == [0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]
        && let Ok(decoded) = ImageData::from_png(raw)
    {
        let soft_mask = decoded.soft_mask.clone();
        return (decoded, soft_mask);
    }

    // JPEG magic: \xFF \xD8 ~keep
    if raw.len() >= 2
        && raw[0] == 0xFF
        && raw[1] == 0xD8
        && let Ok(decoded) = ImageData::from_jpeg(raw.to_vec())
    {
        // JPEG has no alpha channel. ~keep
        return (decoded, None);
    }

    // Pre-processed path: data was already encoded by `ImageData::from_png()`
    // (Flate-compressed pixels with per-row filter bytes) or is raw pixels. ~keep
    let color_space = match image.color_space {
        ...
        // Lab, and no in-crate producer emits them — but a caller can
        // construct an ImageContent with either directly and route it
        // through `PageBuilder::add_element`, and silently coercing
        // that to RGB would produce wrong colours.
        // Fall back to RGB to keep the XObject emittable, and emit a
        // warning so the miscoloration is diagnosable. ~keep
        crate::elements::ColorSpace::Indexed | crate::elements::ColorSpace::Lab => {
            tracing::warn!(
                color_space = ?image.color_space,
                "color space not yet supported by the writer pipeline; falling back to DeviceRGB \
    ...
    // stream straight into the /SMask XObject. ~keep
...
mod tests {
    ...
    fn test_pdf_with_callout() {
        ...
        assert!(content.contains("/Subtype /FreeText"));
        assert!(content.contains("/IT /FreeTextCallout"));
        assert!(content.contains("/CL"));
    }
    ...
    fn test_pdf_with_multiple_ink_annotations() {
        let mut writer = PdfWriter::new();
        {
            ...
            page.ink_styled(vec![(300.0, 100.0), (350.0, 120.0)], (0.0, 0.0, 1.0), 2.0);
            page.finish();
        }
