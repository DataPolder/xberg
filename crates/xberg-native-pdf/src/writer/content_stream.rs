...
pub(crate) fn map_base14_font_name(name: &str, bold: bool) -> String {
    ...
    // governs Base-14 substitution for generic family names. ~keep
    enum Family {
        Helvetica,
        Times,
        Courier,
    ...
    // without also threading a style struct through every layer. ~keep
...
impl ContentStreamBuilder {
    ...
    pub fn circle(&mut self, cx: f32, cy: f32, radius: f32) -> &mut Self {
        // Bézier approximation constant for circles ~keep
        let k = 0.552_284_8;
        let c = radius * k;
    ...
    pub fn add_element(&mut self, element: &ContentElement) -> &mut Self {
        match element {
            ...
            ContentElement::Structure(s) => {
                ...
                // so PdfWriter::finish can build the StructTreeRoot. ~keep
    ...
    fn add_text_content(&mut self, text: &TextContent) -> &mut Self {
        ...
        // so Assistive Technology skips it. ~keep
        let is_artifact = text.artifact_type.is_some();
        if is_artifact {
            use crate::extractors::text::ArtifactType;
            // End any open text object before BDC (BDC must be outside BT/ET). ~keep
            self.end_text();
        ...
        // exactly the "no text at all" symptom we hit on XLSX→PDF tables. ~keep
    ...
    fn add_path_content(&mut self, path: &PathContent) -> &mut Self {
        ...
        // The `had_matrix` flag drives the matching `Q` after the
        // stroke/fill op below. ~keep
        ...
        // operations; this is safer than assuming a surrounding q/Q.) ~keep
        ...
        // reader doesn't think we're leaking dash state on transforms. ~keep
        if had_matrix {
            self.op(ContentStreamOp::RestoreState);
        }
    ...
    fn add_table_content(&mut self, table: &TableContent) -> &mut Self {
        ...
        for (row_idx, row) in table.rows.iter().enumerate() {
            ...
            if row_idx % 2 == 1
                && let Some((r, g, b)) = style.stripe_background
            {
                self.op(ContentStreamOp::SetFillColorRGB(r, g, b));
                self.op(ContentStreamOp::Rectangle(
                    table.bbox.x,
            ...
            if row.is_header
                && let Some((r, g, b)) = style.header_background
            {
                self.op(ContentStreamOp::SetFillColorRGB(r, g, b));
                self.op(ContentStreamOp::Rectangle(
                    table.bbox.x,
            ...
            for (col_idx, cell) in row.cells.iter().enumerate() {
                ...
                if !cell.text.is_empty() {
                    ...
                    self.begin_text();
                    self.op(ContentStreamOp::SetFillColorRGB(0.0, 0.0, 0.0));
    ...
    fn add_image_content(&mut self, image: &ImageContent) -> &mut Self {
        ...
        // PDF/UA-1 F-1: images with alt text → /Figure BDC/EMC + StructElemRecord. ~keep
        ...
        if has_alt {
            ...
            // StructElem with /Alt when assembling the StructTreeRoot. ~keep
    ...
    fn write_escaped_string<W: Write>(&self, w: &mut W, text: &str) -> std::io::Result<()> {
        for ch in text.chars() {
            ...
            // for word-level text recovery and only loses the styling. ~keep
            ...
            // degrades to `?` — those need an embedded Unicode font. ~keep
...
mod tests {
    ...
    fn test_font_mapping_explicit_standard14() {
        ...
        // need Symbol/ZapfDingbats must use the embedded-font path. ~keep
    ...
    fn test_mixed_content_elements() {
        ...
        assert!(content.contains("(Row 1) Tj"));
    ...
    fn test_set_font_caching() {
        ...
        builder.set_font("Helvetica", 12.0);
        builder.set_font("Helvetica", 14.0);
        builder.end_text();
    ...
    fn test_font_mapping_serif() {
        ...
        // embedded resource, so bold serif text rendered as regular. ~keep
