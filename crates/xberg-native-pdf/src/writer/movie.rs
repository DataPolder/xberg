...
//!
//! ```ignore
//! use xberg_native_pdf::writer::MovieAnnotation;
...
impl MovieData {
    ...
    pub fn build_movie_dict(&self, file_spec_ref: ObjectRef) -> HashMap<String, Object> {
        ...
        // F - file specification (required) ~keep
        ...
        // Poster - use true to indicate poster should be retrieved from movie ~keep
        if self.poster.is_some() {
            // If we have poster data, it needs to be written as a stream reference
            // For now, use true to indicate the viewer should extract from movie ~keep
...
impl MovieAnnotation {
    ...
    pub fn build(&self, _page_refs: &[ObjectRef]) -> HashMap<String, Object> {
        ...
        // A - Activation parameters (if not default) ~keep
        ...
        // Note: The Movie entry must be added by the caller after writing the Movie dict ~keep
