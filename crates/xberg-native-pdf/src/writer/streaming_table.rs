//! Streaming table surface for `FluentPageBuilder`.
//!
...
/// requires looking at rows the caller hasn't pushed yet.
#[derive(Debug, Clone)]
...
impl<'a> StreamingTable<'a> {
    ...
    pub(super) fn open(page: FluentPageBuilder<'a>, config: StreamingTableConfig) -> Self {
        ...
        let sample_state = match &config.mode {
            ...
            // AutoAll is detected lazily in push_row and surfaces an error there. ~keep
        ...
        F: FnOnce(&mut StreamingRow),
    {
        ...
        // Extract plain text for sample-mode buffering (rowspan ignored in sample). ~keep
        ...
        if should_flush {
            ...
            // inside freeze_and_flush — do not draw it again. ~keep
    ...
    pub fn finish(mut self) -> FluentPageBuilder<'a> {
        ...
        // declared rowspan count was satisfied). ~keep
        if self.rowspan_remaining > 0 {
    ...
    fn draw_row_from(
        ...
        loop {
            ...
            // segments on a new page use top_pad too so each page looks tidy. ~keep
    ...
    fn emit_row_segment(
        ...
        if is_header && let Some((r, g, b)) = self.config.header_fill {
            self.push_path_fill(
                self.origin_x,
                row_top - seg_height,
                self.total_width,
    ...
    fn draw_rowspan_group(&mut self, rows: Vec<Vec<RowCell>>) -> Result<()> {
        ...
        for (row_idx, row_cells) in rows.iter().enumerate() {
            ...
            // (their height is determined by the combined group height). ~keep
            if row_idx == 0 {
        ...
        // for the current page we move it entirely to a new page. ~keep
        if self.page.remaining_space() < total_height {
            ...
            // (it will overflow; cross-page rowspan splitting is future work). ~keep
        }
...
mod tests {
    ...
    fn test_streaming_table_emits_header_and_rows() {
        ...
        // 3 header cells + 3 rows × 3 body cells = 12 text elements. ~keep
    ...
    fn test_streaming_table_page_break_and_repeat_header() {
        // Engineer a near-full page so one more row overflows and forces a break. ~keep
        let mut doc = DocumentBuilder::new();
        let page = doc.letter_page().font("Helvetica", 10.0);
        let page = page.at(72.0, 90.0); // ~18 pt to bottom margin ~keep
        ...
        // 18pt, forces new page. Header must redraw on page 2 before row. ~keep
        t.push_row(|r| {
    ...
    fn test_streaming_table_thirty_thousand_rows_bounded_memory() {
        ...
        // bounded — i.e. the row is consumed and not retained. ~keep
        let mut doc = DocumentBuilder::new();
        ...
        // the important property is no panic, no run-away memory. ~keep
    ...
    fn test_sample_mode_emits_all_rows_and_header() {
        ...
        // 1 header row (2 cells) + 4 body rows (2 cells each) = 10 text elements ~keep
    ...
    fn test_sample_mode_fewer_rows_than_target_flushes_on_finish() {
        ...
        // 1 header + 3 body = 4 elements ~keep
    ...
    fn test_sample_mode_clamps_width_to_min_max() {
        ...
        // multiple text elements, so we only check the header is among them). ~keep
        let texts: Vec<&str> = doc
            .page_elements(0)
            .iter()
            .filter_map(|e| match e {
    ...
    fn test_sample_mode_zero_buffered_uses_column_defaults() {
        ...
        // default to the header text width clamped to [min, max]. ~keep
        let mut doc = DocumentBuilder::new();
        let page = doc.letter_page().font("Helvetica", 10.0).at(72.0, 720.0);
        ...
        // (The header is drawn if at least one row, or on finish if any buffering happened) ~keep
    }
    ...
    fn test_cell_split_across_pages_emits_all_lines() {
        ...
        // The new split logic should move the row to the next page cleanly. ~keep
        let mut doc = DocumentBuilder::new();
        let page = doc.letter_page().font("Helvetica", 10.0).at(72.0, 90.0);
        // After header (~14pt) only ~76pt remain; force extremely limited space. ~keep
    ...
    fn test_tall_cell_splits_mid_cell_across_two_pages() {
        ...
        // wraps into many lines.  Confirm all lines appear somewhere in the doc. ~keep
        let mut doc = DocumentBuilder::new();
        // Place cursor 20 pt from the bottom so the first page has < 1 line of
        // content height after the header. ~keep
