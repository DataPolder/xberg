...
//!
//! ```ignore
//! use xberg_native_pdf::writer::embedded_files::EmbeddedFile;
//!
...
impl EmbeddedFile {
    ...
    pub fn build_stream_dict(&self) -> HashMap<String, Object> {
        ...
        // is disabled we omit the optional field rather than fail. ~keep
    ...
    pub fn build_filespec(&self, embedded_stream_ref: ObjectRef) -> HashMap<String, Object> {
        ...
        // File name (F is the generic name, UF is Unicode name) ~keep
...
fn encode_utf16_be(s: &str) -> Vec<u8> {
    let mut result = vec![0xFE, 0xFF]; // UTF-16BE BOM ~keep
