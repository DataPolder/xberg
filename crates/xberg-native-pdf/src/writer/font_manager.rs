...
fn get_base14_widths(name: &str) -> HashMap<char, f32> {
    ...
    // These are based on standard PostScript font metrics ~keep
...
impl TextLayout {
    ...
    pub fn wrap_text(&self, text: &str, font_name: &str, font_size: f32, max_width: f32) -> Vec<(String, f32)> {
        ...
        if !current_line.is_empty() {
            ...
            // discards that information, so we restore it here. ~keep
...
impl EmbeddedFont {
    ...
    /// the font's CFF byte→GID table (`cff_gid_map` in xberg-native-pdf's
    ...
    pub fn extend_glyph_lookup(&mut self, mapping: HashMap<u32, u16>) {
        // Re-parse once so we can pull widths only for newly-introduced GIDs. ~keep
    ...
    pub fn glyph_id(&self, codepoint: u32) -> Option<u16> {
        ...
        if let Some(base) = crate::fonts::encoding::math_alphanumeric_base(codepoint) {
            ...
            // (e.g. bold dash maps to base then through the dash fallback). ~keep
    ...
    fn glyph_id_fallback(&self, codepoint: u32) -> Option<u16> {
        let fallback = match codepoint {
            ...
            0x00A0 | 0x2007 | 0x202F | 0x2009 | 0x200A => Some(0x20),
    ...
    pub fn encode_shaped_run(
        ...
        for g in &shaped.glyphs {
            ...
            // `use_glyph` (no ToUnicode) rather than panic on bad bounds. ~keep
    ...
    pub fn generate_widths_array(&self, remapper: &crate::fonts::GlyphRemapper) -> String {
        ...
        // with what the content stream actually uses. ~keep
...
mod tests {
    ...
    fn test_line_height() {
        ...
        // For Helvetica: (718 - (-207)) * 12 / 1000 = 11.1 points ~keep
