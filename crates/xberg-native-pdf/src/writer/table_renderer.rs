...
//!
//! ```ignore
//! use xberg_native_pdf::writer::{Table, TableCell, TableStyle};
//!
...
impl Default for TableStyle {
    fn default() -> Self {
        Self {
            ...
            font_size: 10.0,
            header_background: Some((0.9, 0.9, 0.9)),
            stripe_color: None,
            row_spacing: 0.0,
        }
    }
...
impl Table {
    ...
    /// surface. Emitting `ContentElement::Text` + `ContentElement::
    /// Path` rather than writing raw content-stream ops lets the writer's
    /// subsetter (#385) re-key glyph IDs for dynamically-added
    ...
    pub fn to_content_elements(
        ...
        for (row_idx, row) in self.rows.iter().enumerate() {
            for (cell_idx, cell) in row.cells.iter().enumerate() {
                ...
                // Background colour precedence: cell > header-row > stripe > row. ~keep
        ...
        for (row_idx, row) in self.rows.iter().enumerate() {
            for (cell_idx, cell) in row.cells.iter().enumerate() {
                    ...
                    .enumerate()
                {
                    ...
                    // first line sits at the cell's top-after-padding. ~keep
    ...
    pub fn render(
        ...
        for (row_idx, row) in self.rows.iter().enumerate() {
            for (cell_idx, cell) in row.cells.iter().enumerate() {
                ...
                // Get alignment (cell -> column -> default) ~keep
                let align = if cell.align != CellAlign::Left {
                    cell.align
                } else {
                    self.column_aligns
                    ...
                    .enumerate()
                {
                    ...
                    // and Right must offset by this line's measured width. ~keep
                    let text_x = match align {
                        CellAlign::Left => cell_x,
                        CellAlign::Center => cell_x + (content_width - line_width) / 2.0,
                        CellAlign::Right => cell_x + content_width - line_width,
                    ...
                    // top; each subsequent line drops by `line_height`. ~keep
...
impl Default for SimpleFontMetrics {
    fn default() -> Self {
        Self {
            char_width_ratio: 0.5, // Typical for proportional fonts ~keep
        }
    }
}
...
mod tests {
    ...
    fn test_multi_line_cell_layout_stores_wrapped_lines() {
        ...
        // exceeds the minimum `font_size * 1.5` floor. ~keep
    ...
    fn test_to_content_elements_emits_text_per_line() {
        ...
        // One Text per non-empty line per cell (cell 0 wraps, cell 1 is 1 line). ~keep
    ...
    fn test_to_content_elements_path_for_backgrounds_and_borders() {
        ...
        // 4 cells + outer border. Check the fill/stroke mix is plausible. ~keep
        ...
        // order so later overlays paint on top of earlier ones. ~keep
    ...
    fn test_alignment_offsets_per_line_width() {
        ...
        // applied to the MEASURED line width, not the column content width. ~keep
        use super::super::content_stream::ContentStreamBuilder;
        ...
        // Single char "L"/"C"/"R" at default metrics = 1 * 10 * 0.5 = 5.0 ~keep
        let char_w = 5.0f32;
