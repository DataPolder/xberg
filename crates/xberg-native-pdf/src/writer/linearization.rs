...
impl HintTables {
    ...
    fn write_page_offset_table(&self, data: &mut Vec<u8>) {
        ...
        // Write per-page entries (variable bit-packed) ~keep
...
impl BitWriter {
    ...
    fn finish(mut self) -> Vec<u8> {
        if self.bit_position > 0 {
            // Pad remaining bits with zeros ~keep
...
mod tests {
    ...
    fn test_delta_encoding() {
        ...
        assert_eq!(min, 10);
        assert_eq!(bits, 4); // max delta is 15, needs 4 bits ~keep
    }
