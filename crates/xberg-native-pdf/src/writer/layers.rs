...
//!
//! ```ignore
//! use xberg_native_pdf::writer::layers::{LayerBuilder, LayerVisibility};
//!
//! // Create layers
...
impl Layer {
    ...
    pub fn build_usage_dict(&self) -> Option<HashMap<String, Object>> {
        // Only create usage dict if we have non-default settings ~keep
...
impl LayerBuilder {
    ...
    pub fn build_oc_properties(&self, ocg_refs: &[ObjectRef]) -> HashMap<String, Object> {
        ...
        // D (default) configuration dictionary ~keep
        ...
        // OFF array - layers that are hidden by default (when BaseState is ON) ~keep
...
mod tests {
    ...
    fn test_layer_builder_base_state() {
        ...
        assert_eq!(builder.creator, Some("xberg-native-pdf".to_string()));
    }
    ...
    fn test_build_oc_properties_with_creator() {
        let mut builder = LayerBuilder::new().creator("native_pdf_test");
        builder.add_layer("Layer1");
        ...
        if let Some(Object::Dictionary(d_dict)) = props.get("D") {
            if let Some(Object::String(creator_bytes)) = d_dict.get("Creator") {
                assert_eq!(creator_bytes, b"native_pdf_test");
            } else {
    ...
    fn test_build_oc_properties_more_layers_than_refs() {
        ...
        if let Some(Object::Dictionary(d_dict)) = props.get("D")
            && let Some(Object::Array(on_arr)) = d_dict.get("ON")
        {
            assert_eq!(on_arr.len(), 1);
        }
    }
    ...
    fn test_layer_membership_ocmd_single_ocg() {
        ...
        // Single OCG should be a Reference, not an Array ~keep
    ...
    fn test_layer_membership_ocmd_multiple_ocgs() {
        ...
        // Multiple OCGs should be an Array ~keep
