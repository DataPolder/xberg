...
//!
//! Real font subsetting (FONT-3b): `FontFile2` carries the
...
pub fn build_embedded_font_objects(
    ...
    // produces the same subset name (helps reproducible builds). ~keep
    ...
    // larger PDFs but the source typeface program reaches the viewer. ~keep
    let (font_bytes, remapper) = match crate::fonts::subset_font_bytes(font.font_data(), 0, font.used_glyphs()) {
        Ok(pair) => pair,
        Err(e) => {
            tracing::warn!(
                font_name = %font.name,
                bytes = font.font_data().len(),
                error = %e,
                "subsetting failed; embedding full font"
            );
    ...
    if is_cff {
        ...
        // CIDFontType0C signals CFF-based CID font data. ~keep
        ff_dict.insert("Subtype".to_string(), ObjectSerializer::name("CIDFontType0C"));
    } else {
        // Length1 is required for FontFile2 (TrueType) per PDF spec §9.9. ~keep
    ...
    // CFF fonts reference the stream via FontFile3; TrueType via FontFile2. ~keep
    ...
    // must not appear for CIDFontType0 (bug #449). ~keep
    ...
    let mut cidfont_entries: Vec<(&str, Object)> = vec![
        ...
        // /W carries glyph widths indexed by CID/GID. ~keep
        ("W", parse_widths_string_to_array(&widths_str)),
    ];
    if !is_cff {
        // CIDToGIDMap /Identity: source GIDs are the CIDs (TrueType only). ~keep
    ...
    // ── 5. Type 0 wrapper (§9.6.4, Table 110) ──────────────────────────── ~keep
...
/// helpers can stay as `String`-returning APIs (their original
...
fn parse_widths_string_to_array(s: &str) -> Object {
    ...
    flush_number(&mut stack, &mut number);
    // The outer stack holds [[ <root array> ]]; the root array is what we want. ~keep
...
mod tests {
    ...
    fn parse_widths_simple() {
        ...
        // Outer is an array of: integer 36, inner-array [600,600], integer 65, inner [720]. ~keep
        match parsed {
