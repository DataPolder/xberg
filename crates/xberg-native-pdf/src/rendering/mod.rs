...
pub fn flatten_to_images(doc: &crate::document::PdfDocument, dpi: u32) -> Result<Vec<u8>> {
    ...
    // Keyed on a per-call sequence as well as the process id: keyed on the pid
    // alone, two threads flattening at once wrote into one directory and each
    // one's remove_dir_all deleted the other's pages mid-run. ~keep
    static FLATTEN_SEQUENCE: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
    let tmp_dir = std::env::temp_dir().join(format!(
        "native_pdf_flatten_{}_{}",
        std::process::id(),
        FLATTEN_SEQUENCE.fetch_add(1, std::sync::atomic::Ordering::Relaxed)
    ));
