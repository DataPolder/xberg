...
impl GfmTable {
    ...
    fn parse(lines: &[&str]) -> Option<Self> {
        ...
        for line in &lines[2..] {
            if let Some(row) = Self::parse_row(line) {
                // Pad or truncate row to match header column count ~keep
    ...
    fn parse_separator(line: &str, expected_cols: usize) -> Option<Vec<GfmAlign>> {
        ...
        // Pad with default alignment if needed ~keep
    ...
    fn column_widths(&self) -> Vec<usize> {
        ...
        // Minimum width of 3 for readability ~keep
...
impl Pdf {
    ...
    /// Extract tables from a page with custom configuration.
    ...
    /// Create a focused view of a page region for scoped extraction.
    ///
    /// # Example
    ///
    /// ```ignore
    ...
    pub fn highlight_matches(&mut self, results: &[crate::search::SearchResult], color: [f32; 3]) -> Result<()> {
        ...
        // Group results by page for efficiency ~keep
    ...
    pub fn to_bytes(&mut self) -> Result<Vec<u8>> {
        if let Some(ref mut editor) = self.editor {
            // Serialises through the same write_full_to_writer as save(), without
            // the filesystem round-trip: the old temp path was keyed only on the
            // process id, so two threads calling to_bytes() at once truncated and
            // read back each other's file. It also could not work on wasm32,
            // where std::fs returns Err(Unsupported). ~keep
    ...
    ///
    /// ```ignore
    /// use xberg_native_pdf::writer::{EmbeddedFile, AFRelationship};
    ///
...
/// ```
/// A focused view of a PDF page region for scoped extraction.
///
...
impl PdfBuilder {
    ...
    fn render_images(&self, images: &[crate::writer::ImageData]) -> Result<Vec<u8>> {
        ...
        config.creator = Some("xberg-native-pdf".to_string());
        ...
        for image in images {
            ...
            let mut image_content = ImageContent {
                ...
                // render with their alpha channel intact. ~keep
    ...
    fn render_markdown_with_fonts(&self, content: &str, extra_fonts: &[(String, Vec<u8>)]) -> Result<Vec<u8>> {
        ...
        if needs_unicode {
            ...
            // (the Base-14 Helvetica-Bold only covers WinAnsi). ~keep
            if let Ok(font) = crate::writer::EmbeddedFont::from_data(
        ...
        // DejaVu is bundled); headings/bold still apply correctly. ~keep
        //
        ...
        // can fan out into several differently-styled runs sharing a `y`. ~keep
        struct LaidRun {
            x0: f32,
            y: f32,
            text: String,
        ...
        while i < lines.len() {
            ...
            if !in_code && is_table_line(line) && i + 1 < lines.len() {
                ...
                // column padding actually lines up. ~keep
            ...
            // `heading_bold` renders the whole line in the bold face. ~keep
            ...
            // parsing (back-ticks etc. are literal there). ~keep
        ...
        // so inline style changes stay visually contiguous. ~keep
        {
    ...
    fn render_html(&self, content: &str) -> Result<Vec<u8>> {
        // Simple HTML to Markdown conversion, then render as Markdown ~keep
...
mod tests {
    ...
    fn test_parse_inline_runs_basic_and_edges() {
        ...
        // Code span wins over emphasis: markers inside are literal. ~keep
        assert_eq!(enc("`a*b*c`"), "C:a*b*c");

        // Degenerate inputs produce no runs (and never panic). ~keep
        assert_eq!(enc(""), "");
        assert_eq!(enc("****"), "");
    }
...
"#;
    ...
    /// `to_bytes()` used to serialise through `$TMPDIR/native_pdf_temp_<pid>.pdf`,
    /// a path keyed only on the process id. Two threads in one process therefore
    /// wrote and read back the same file, and a caller could silently receive
    /// another document's bytes — the worst failure mode for a parser, because
    /// the result still looks like a valid PDF. ~keep
    #[test]
    fn to_bytes_returns_each_callers_own_document_under_concurrency() {
        ...
        // ordering in the output varies between runs, so two serialisations of
        // one document are not byte-identical. ~keep
        let handles: Vec<_> = sources
            .into_iter()
            .enumerate()
            .map(|(index, source)| {
                std::thread::spawn(move || {
                    let marker = format!("document number {index}");
                    for _ in 0..ITERATIONS {
                        let mut pdf = Pdf::from_bytes(source.clone()).unwrap();
                        let produced = pdf.to_bytes().unwrap();
                        assert!(
                            produced.starts_with(b"%PDF"),
