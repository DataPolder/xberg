...
fn test_permissions_reserved_bits() {
    // Per PDF spec, bits 7-8 and 13-32 must be 1 ~keep
    let perms = Permissions::default();
    let bits = perms.to_bits();
    // Bits 6-7 (0-indexed) must be set ~keep
    assert!(bits & (1 << 6) != 0, "Bit 7 must be set");
    assert!(bits & (1 << 7) != 0, "Bit 8 must be set");
    // High bits (12-31) must be set ~keep
...
fn test_editor_save_with_encryption_options() {
    ...
    let (path_dir, path) = common::write_temp_pdf(&pdf, "enc_save_src.pdf");
    let out_path = path_dir.path().join("enc_save_out.pdf");
