//! Regression tests for `DocumentBuilder` encryption.
//!
//! `Pdf::save_encrypted` used to work only for PDFs
...
//!
//! `DocumentBuilder` now exposes `save_encrypted`, `save_with_encryption`,
//! and `to_bytes_encrypted` / `to_bytes_with_encryption`.
//! Each routes the built bytes through
...
use xberg_native_pdf::editor::{EncryptionAlgorithm, EncryptionConfig, Permissions};
...
fn save_encrypted_produces_aes256_encrypt_dict() {
    ...
    make_builder("confidential content")
...
fn save_with_encryption_respects_restricted_permissions() {
    ...
    // sits between `/P ` and the next whitespace. ~keep
    ...
    // shift is 0-based, so bit 3 = (1 << 2), bit 4 = (1 << 3), etc. ~keep
...
/// halves end to end: font subsetting produces the bytes, encryption
/// wraps them. This guards against any layering bug where the
/// encryption path re-parses the build output and fails to handle the
/// content-stream ops emitted for subset fonts.
#[test]
fn save_encrypted_works_with_embedded_font_subsetting() {
    use std::path::Path;
    use xberg_native_pdf::writer::EmbeddedFont;

    let dir = tempdir().unwrap();
    let path = dir.path().join("out_enc_embedded.pdf");

    let font = EmbeddedFont::from_file(Path::new("src/fonts/assets/DejaVuSans.ttf"))
...
// DescendantFonts / FontFile2 / ToUnicode for embedded TrueType fonts). ~keep
...
/// pipeline.  This is the core content round-trip regression.
/// Uses AES-128 which fully round-trips today.
#[test]
fn save_encrypted_embedded_ttf_content_preserved() {
    ...
    let mut builder = DocumentBuilder::new()
        .metadata(DocumentMetadata::new().title("encrypted embedded font regression"))
        .register_embedded_font("DejaVu", font);
    builder
        .a4_page()
        .font("DejaVu", 12.0)
    ...
    assert!(
        !text.is_empty(),
        "text extracted from embedded-font encrypted PDF must not be empty"
    );
    assert!(
...
/// the reported scenario as closely as possible without
...
fn two_embedded_fonts_aes128_content_preserved() {
    ...
    let dir = tempdir().unwrap();
    let path = dir.path().join("two_embedded_fonts.pdf");

    let font_regular = EmbeddedFont::from_file(Path::new("src/fonts/assets/DejaVuSans.ttf"))
        .expect("DejaVuSans.ttf fixture must be present");
    let font_bold = EmbeddedFont::from_file(Path::new("src/fonts/assets/DejaVuSans-Bold.ttf"))
    ...
    assert!(
        !text.is_empty(),
        "content must not be empty after encryption with embedded fonts; \
    ...
    assert!(
        ...
        "extracted text {text:?} should contain recognisable words \
...
fn save_encrypted_aes256_embedded_font_size_check() {
    ...
    // accounting for compression (actual diff observed: ~8 KB). ~keep
    assert!(
        diff >= 3_000,
...
fn save_encrypted_aes256_ffi_sequence_embedded_font() {
    use std::path::Path;
    use xberg_native_pdf::writer::{DocumentBuilder, EmbeddedFont, PageSize};
    ...
    // compress=true shrinks the raw FontFile2 stream; 3 KB floor is sufficient. ~keep
    assert!(
        diff >= 3_000,
...
fn debug_trace_editor_from_bytes_size() {
    ...
    // A 5 KB floor clearly distinguishes "font present" from "skeleton only". ~keep
    assert!(
        enc_size > 5_000,
...
fn all_object_ids_returns_complete_set_for_built_pdf() {
    ...
    for id in &ids {
        ...
        let obj = doc.load_object(xberg_native_pdf::object::ObjectRef::new(*id, 0));
        eprintln!("  Object {}: {:?}", id, obj.as_ref().map(|_| "ok").unwrap_or("ERR"));
        // We don't assert success here because some free objects return Null ~keep
    }
}
