...
//!
//! Reproduces the reported symptom: a `/v1/forms/fill` with a CJK value produced an
...
use xberg_native_pdf::extractors::forms::{FieldValue, FormExtractor};
...
fn merged_widget_form() -> Vec<u8> {
    let objs: [&str; 5] = [
        ...
        // (/Subtype /Widget /Rect /P) on one object. ~keep
