//! Round-trip tests for embedded TrueType fonts.
//!
...
//! and asserts that `extract_text` returns the input. If the
...
use xberg_native_pdf::writer::{EmbeddedFont, PdfWriter};
...
fn build_pdf_with_text(text: &str, font_size: f32) -> Vec<u8> {
    ...
    let font =
        EmbeddedFont::from_data(Some("DejaVuSans".to_string()), DEJAVU_SANS.to_vec()).expect("DejaVuSans must parse");
...
fn round_trip_latin_extended() {
    ...
    // 0x7f. DejaVuSans has full Latin Extended coverage. ~keep
...
fn round_trip_cyrillic() {
    ...
    // codepoint range entirely above U+0400. DejaVuSans covers Cyrillic. ~keep
