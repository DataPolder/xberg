//! Regression tests for real font subsetting on the
//! `DocumentBuilder::register_embedded_font` path.
//!
//! The `subsetter` wrapper at
//! `crate::fonts::subset_font_bytes` used to be present and `FontSubsetter`
...
fn ascii_document_is_much_smaller_than_original_face() {
    ...
    // very wrong regardless of subsetting. ~keep
    ...
    // regression to full-font embedding (760 KB). ~keep
    assert!(
        bytes.len() < 50_000,
...
/// were actually reporting breakage against. DejaVuSans has cmap
...
fn two_embedded_fonts_produce_independent_subsets() {
    ...
    // runs (cross-applying would produce garbled text). ~keep
    ...
    // full face worth of glyphs and blow up the total size. ~keep
    let face_bytes = dejavu_byte_len();
    assert!(
        bytes.len() < 80_000,
...
fn output_pdf_carries_subset_tag_in_font_name() {
    ...
    // letters — the canonical PDF subset-tag prefix (ISO 32000-1 §9.6.4). ~keep
