...
use xberg_native_pdf::writer::{AFRelationship, EmbeddedFile};
...
mod embed_file_api {
    ...
    fn test_embed_file_simple() {
        ...
        let input_path = temp_dir.path().join("test_embed_input.pdf");
        let output_path = temp_dir.path().join("test_embed_output.pdf");
    ...
    fn test_embed_file_with_options() {
        ...
        let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
        let input_path = temp_dir.path().join("test_embed_opts_input.pdf");
        let output_path = temp_dir.path().join("test_embed_opts_output.pdf");
        std::fs::write(&input_path, &bytes).expect("Failed to write temp PDF");
    ...
    fn test_embed_multiple_files() {
        ...
        let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
        let input_path = temp_dir.path().join("test_embed_multi_input.pdf");
        let output_path = temp_dir.path().join("test_embed_multi_output.pdf");
        std::fs::write(&input_path, &bytes).expect("Failed to write temp PDF");
        ...
        assert!(output_bytes.len() > bytes.len());
    ...
    fn test_clear_embedded_files() {
        ...
        let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
        let input_path = temp_dir.path().join("test_embed_clear_input.pdf");
        std::fs::write(&input_path, &bytes).expect("Failed to write temp PDF");
    ...
    fn test_embed_binary_file() {
        ...
        let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
        let input_path = temp_dir.path().join("test_embed_binary_input.pdf");
        let output_path = temp_dir.path().join("test_embed_binary_output.pdf");
        std::fs::write(&input_path, &bytes).expect("Failed to write temp PDF");
...
mod embedded_files_builder {
    use super::*;
    use xberg_native_pdf::writer::EmbeddedFilesBuilder;
