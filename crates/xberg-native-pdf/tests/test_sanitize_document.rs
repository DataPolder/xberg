//! Standalone document sanitization integration test (feature plan
//! §4.6 / §7 G5+G6): build a real PDF carrying secrets in
...
use xberg_native_pdf::{PdfDocument, RedactionOptions};
...
fn sanitize_document_strips_info_and_embedded_file_secrets() {
    ...
    // string before sanitization, else the oracle proves nothing. ~keep
    assert!(
    ...
    // construct, and the dropped objects have non-zero size. ~keep
    assert!(
    ...
    // if it was present pre-sanitization it must be absent afterwards. ~keep
    ...
    // public body text is intact (sanitize is non-geometric). ~keep
