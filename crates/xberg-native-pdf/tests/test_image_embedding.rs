...
use xberg_native_pdf::writer::{
    ColorSpace, ContentStreamBuilder, ImageData, ImageFormat, ImageManager, ImagePlacement, PdfWriter, PdfWriterConfig,
};
...
const MINIMAL_JPEG: &[u8] = &[
    ...
    0x01, 0x00, 0x00, 0x3F, 0x00, 0xFB, 0xD5, 0xDB, 0x20, 0xA8, 0xF1, 0x47, 0xFF, 0xD9,
];
...
fn create_test_png_rgba(width: u32, height: u32) -> Vec<u8> {
    ...
    for _ in 0..height {
        raw_pixels.push(0);
        for _ in 0..width {
            raw_pixels.extend_from_slice(&[255, 0, 0, 128]);
        }
    }
...
fn create_test_png(width: u32, height: u32) -> Vec<u8> {
    ...
    for _ in 0..height {
        raw_pixels.push(0);
        for _ in 0..width {
            raw_pixels.extend_from_slice(&[255, 0, 0]);
        }
    }
...
mod image_data_tests {
    ...
    fn test_xobject_dict_raw() {
        ...
        assert_eq!(
            dict.get("Type"),
            Some(&xberg_native_pdf::object::Object::Name("XObject".to_string()))
        );
        assert_eq!(
            dict.get("Subtype"),
            Some(&xberg_native_pdf::object::Object::Name("Image".to_string()))
        );
        assert_eq!(dict.get("Width"), Some(&xberg_native_pdf::object::Object::Integer(100)));
        assert_eq!(dict.get("Height"), Some(&xberg_native_pdf::object::Object::Integer(50)));
        assert_eq!(
            dict.get("ColorSpace"),
            Some(&xberg_native_pdf::object::Object::Name("DeviceGray".to_string()))
        );
    ...
    fn test_xobject_dict_jpeg() {
        ...
        assert_eq!(
            dict.get("Filter"),
            Some(&xberg_native_pdf::object::Object::Name("DCTDecode".to_string()))
        );
    }
    ...
    fn test_xobject_dict_png() {
        ...
        assert_eq!(
            dict.get("Filter"),
            Some(&xberg_native_pdf::object::Object::Name("FlateDecode".to_string()))
        );
...
mod image_manager_tests {
    ...
    fn test_register_multiple_images() {
        ...
        let id2 = manager.register("image2", ImageData::new(20, 20, ColorSpace::DeviceGray, vec![]));
        let id3 = manager.register("image3", ImageData::new(30, 30, ColorSpace::DeviceCMYK, vec![]));
    ...
    fn test_resource_id() {
        let mut manager = ImageManager::new();
        let id = manager.register("my_image", ImageData::new(10, 10, ColorSpace::DeviceRGB, vec![]));
...
mod compression_tests {
    ...
    fn test_compressed_pdf_is_smaller() {
        ...
        // Note: For very small content, compression overhead might make it larger ~keep
        assert!(
...
mod image_dedup_tests {
    ...
    /// Build a small PDF that places the same PNG bytes twice.
    ...
    fn test_identical_images_deduplicated() {
        ...
        // Standard-14 set is unconditional. ~keep
        let threshold = png_size + 2560;
        assert!(
    ...
    fn test_different_images_not_merged() {
        ...
        let pdf_bytes = builder.build().expect("build");
        // Both images together: must be larger than a single image + small overhead ~keep
        assert!(
...
/// Regression tests for the diagonal-line artifact in images with transparency.
///
...
mod soft_mask_decode_parms_tests {
    ...
    use xberg_native_pdf::writer::ImageData;
