...
mod aes256_encryption_tests {
    ...
    fn test_encrypt_pdf_aes256() {
        ...
        assert!(output_str.contains("/V 5"));
    }
    ...
    fn test_encrypt_pdf_aes256_structure() {
        ...
        assert!(output_str.contains("/V 5"));
        assert!(output_str.contains("/R 6"));
        assert!(output_str.contains("/O "));
        assert!(output_str.contains("/U "));
    ...
    // For now, we test that encrypted PDFs are created with correct structure. ~keep
}
...
mod aes128_encryption_tests {
    ...
    fn test_encrypt_pdf_aes128() {
        ...
        let output_str = read_file_as_string(&output_path);
        assert!(output_str.contains("/Encrypt"));
        assert!(output_str.contains("/V 4"));
        assert!(output_str.contains("/R 4"));
    }

    // Note: Round-trip tests with password authentication require
    // full password verification implementation on the read side. ~keep
}
...
mod rc4_encryption_tests {
    ...
    fn test_encrypt_pdf_rc4_128() {
        ...
        assert!(output_str.contains("/R 3"));
    }
    ...
    fn test_encrypt_pdf_rc4_40() {
        ...
        let output_str = read_file_as_string(&output_path);
        assert!(output_str.contains("/Encrypt"));
        assert!(output_str.contains("/V 1"));
        assert!(output_str.contains("/R 2"));
    }
