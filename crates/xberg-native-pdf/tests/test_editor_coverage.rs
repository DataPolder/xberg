...
fn test_document_info_from_empty_dict() {
    let obj = xberg_native_pdf::object::Object::Dictionary(std::collections::HashMap::new());
...
fn test_document_info_from_non_dict() {
    let obj = xberg_native_pdf::object::Object::Integer(42);
    let info = DocumentInfo::from_object(&obj);
    assert!(info.title.is_none());
}
...
fn test_editor_open() {
    ...
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_open.pdf");
...
fn test_editor_page_count() {
    ...
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_pages.pdf");
...
fn test_editor_set_title() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_title.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_author() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_author.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_subject_keywords() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_meta.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_metadata_roundtrip() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_roundtrip.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_source_path() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_source.pdf");
    let editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_current_page_count() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_current_pages.pdf");
    let editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_info() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_pageinfo.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_save_full_rewrite() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_save_src.pdf");
    let (_out_path_dir, out_path) = common::write_temp_pdf(&[], "editor_save_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_save_default() {
    let pdf = build_minimal_pdf();
    let (path_dir, path) = common::write_temp_pdf(&pdf, "editor_save_default_src.pdf");
    let out_path = path_dir.path().join("editor_save_default_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_rotation() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_rotation.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_rotate_page_by() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_rotate_by.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_rotate_all_pages() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_rotate_all.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_media_box() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_mediabox.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_crop_box_default_none() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_cropbox.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_crop_margins() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_crop_margins.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_erase_regions_multiple() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_erase_multi.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
    editor
...
fn test_editor_flatten_annotations() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_flatten.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_flatten_all_annotations() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_flatten_all.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_unmark_flatten() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_unmark.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
    editor.flatten_page_annotations(0).unwrap();
    assert!(editor.is_page_marked_for_flatten(0));
...
fn test_editor_flatten_forms_on_page() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_flatten_forms.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_flatten_forms_all() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_flatten_forms_all.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_clear_embedded_files() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_clear_embed.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_document_trait() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_trait.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_redaction_marking() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_redact.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_apply_all_redactions() {
    ...
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_redact_all.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_image_modifications_empty() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_images.pdf");
    let editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_has_xfa_simple_pdf() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_xfa.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_remove_page() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_remove_page.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_remove_page_out_of_range() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_remove_oob.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_remove_all_pages_one_by_one() {
    let pdf = build_multi_page_pdf(2);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_remove_all.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_move_page() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_move_page.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_move_page_same_position() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_move_same.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_move_page_out_of_range() {
    let pdf = build_multi_page_pdf(2);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_move_oob.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_duplicate_page() {
    let pdf = build_multi_page_pdf(2);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_dup_page.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_duplicate_page_out_of_range() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_dup_oob.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editable_duplicate_then_remove() {
    let pdf = build_multi_page_pdf(2);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editable_dup_remove.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_merge_from_same_file() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_merge_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_merge_from_multi_page() {
    ...
    let (_path1_dir, path1) = common::write_temp_pdf(&pdf1, "editor_merge_main.pdf");
    let (_path2_dir, path2) = common::write_temp_pdf(&pdf2, "editor_merge_append.pdf");
...
fn test_editor_merge_pages_from_specific() {
    ...
    let (_path1_dir, path1) = common::write_temp_pdf(&pdf1, "editor_merge_pages_main.pdf");
    let (_path2_dir, path2) = common::write_temp_pdf(&pdf2, "editor_merge_pages_src.pdf");
    let mut editor = DocumentEditor::open(&path1).unwrap();
...
fn test_editor_merge_pages_from_empty() {
    ...
    let (_path1_dir, path1) = common::write_temp_pdf(&pdf1, "editor_merge_pages_empty_main.pdf");
    let (_path2_dir, path2) = common::write_temp_pdf(&pdf2, "editor_merge_pages_empty_src.pdf");
    let mut editor = DocumentEditor::open(&path1).unwrap();
...
fn test_editor_merge_pages_from_out_of_range() {
    let pdf1 = build_minimal_pdf();
    let pdf2 = build_multi_page_pdf(2);
    let (_path1_dir, path1) = common::write_temp_pdf(&pdf1, "editor_merge_pages_oob_main.pdf");
    let (_path2_dir, path2) = common::write_temp_pdf(&pdf2, "editor_merge_pages_oob_src.pdf");
    let mut editor = DocumentEditor::open(&path1).unwrap();
...
fn test_editor_extract_pages_works() {
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_extract.pdf");
    let (_out_path_dir, out_path) = common::write_temp_pdf(&[], "editor_extract_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_extract_pages_invalid_index() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_extract_inv.pdf");
    let (_out_path_dir, out_path) = common::write_temp_pdf(&[], "editor_extract_inv_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_returns_pdf_page() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_root_element() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_root.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_children() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_children.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_save_page_roundtrip() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_save_page_rt.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_find_text() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_text.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    let page = editor.get_page(0).unwrap();
    let texts = page.find_text_containing("Test");
    // May or may not find text depending on extraction, but should not panic ~keep
    let _ = texts;
...
fn test_editor_get_page_find_images() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_images.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_find_paths() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_paths.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_find_tables() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_tables.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_annotations_empty() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_page_annots.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_editor_create_and_done() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_page_editor.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_editor_find_text() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_pe_find_text.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_editor_find_images() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_pe_find_img.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_editor_find_paths() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_pe_find_paths.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_editor_find_tables() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_pe_find_tables.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_edit_page_closure() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_edit_page.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    editor.edit_page(0, |_page| Ok(())).unwrap();
    assert!(editor.is_modified());
...
fn test_editor_get_form_fields_empty() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_form_empty.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_has_form_field_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_has_form.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_value_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_set_form_val.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    let result = editor.set_form_field_value(
        "nonexistent",
        xberg_native_pdf::editor::FormFieldValue::Text("test".into()),
    );
...
fn test_editor_get_form_field_value_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_form_val.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_remove_form_field_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_rm_form.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_readonly_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_readonly.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_required_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_required.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_tooltip_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_tooltip.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_max_length_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_maxlen.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_alignment_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_align.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_background_color_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_bgcolor.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_border_color_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_bdrcolor.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_border_width_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_bdrwidth.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_default_appearance_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_da.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_flags_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_flags.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_set_form_field_rect_not_found() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_ff_rect.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    let result = editor.set_form_field_rect(
        "nonexistent",
        xberg_native_pdf::geometry::Rect::new(0.0, 0.0, 100.0, 20.0),
    );
    assert!(result.is_err());
...
fn test_editor_get_page_content() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_get_content.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    let content = editor.get_page_content(0).unwrap();
    // Should return Some or None for a valid page ~keep
    let _ = content;
...
fn test_editor_set_page_content() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_set_content.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    let structure = xberg_native_pdf::elements::StructureElement {
        structure_type: "Document".to_string(),
        bbox: xberg_native_pdf::geometry::Rect::new(0.0, 0.0, 612.0, 792.0),
        children: Vec::new(),
...
fn test_editor_set_page_content_out_of_range() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_set_content_oob.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();

    let structure = xberg_native_pdf::elements::StructureElement::default();
...
fn test_editor_modify_structure() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_modify_struct.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
    ...
    // This may succeed or fail depending on whether structure is available ~keep
    let _ = result;
...
fn test_editor_modify_structure_change_language() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_modify_struct_lang.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_images_empty() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_page_images.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_get_page_images_out_of_range() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_page_images_oob.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_save_incremental() {
    ...
    // (the incremental writer searches backwards from len-100 to 0) ~keep
    let pdf = build_multi_page_pdf(3);
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_save_incr_src.pdf");
    let (_out_path_dir, out_path) = common::write_temp_pdf(&[], "editor_save_incr_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
    ...
    // search range does not cover the keyword; exercise the code path either way ~keep
    let _ = result;
...
fn test_editor_save_with_compression() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_save_compress_src.pdf");
    let (_out_path_dir, out_path) = common::write_temp_pdf(&[], "editor_save_compress_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_save_no_compress_no_gc() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_save_nocomp_src.pdf");
    let (_out_path_dir, out_path) = common::write_temp_pdf(&[], "editor_save_nocomp_out.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_source_immutable() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_source_imm.pdf");
    let editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_source_mutable() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_source_mut.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_resource_manager_access() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_resmgr.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_has_modified_annotations_false() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_has_mod_annots.pdf");
    let editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_info_all_pages() {
    ...
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_all_page_info.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_page_info_out_of_range() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_page_info_oob.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_flatten_forms_will_remove_acroform() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_flatten_forms_acroform.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_editor_combined_rotate_and_crop() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "editor_combined_rot_crop.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_save_to_bytes_returns_valid_pdf() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "save_to_bytes_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_save_to_bytes_with_compress_produces_valid_pdf() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "compress_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_save_to_bytes_with_garbage_collect_produces_valid_pdf() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "gc_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_save_to_bytes_compress_and_gc_together() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "compress_gc_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_gc_produces_smaller_or_equal_output_than_no_gc() {
    ...
    // GC output is <= no-gc output in size (orphans removed). ~keep
    let mut pdf = b"%PDF-1.7\n".to_vec();
    let off1 = pdf.len();
    pdf.extend_from_slice(b"1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n");
    let off2 = pdf.len();
    ...
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "gc_size_src.pdf");
...
fn test_compress_produces_smaller_or_equal_output_for_raw_streams() {
    ...
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "compress_size_src.pdf");
...
fn test_linearize_true_does_not_panic() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "linearize_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
...
fn test_save_to_bytes_round_trip_preserves_content() {
    let pdf = build_minimal_pdf();
    let (_path_dir, path) = common::write_temp_pdf(&pdf, "rt_src.pdf");
    let mut editor = DocumentEditor::open(&path).unwrap();
