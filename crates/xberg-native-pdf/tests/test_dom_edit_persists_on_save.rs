//! Regression tests for `PdfDocument.save()`/`to_bytes()` silently
//! dropping DOM edits (`PdfPage::set_text`, `remove_element`) made to
...
//!
//! Also covers a `/Contents` array-merge bug found while fixing that: when a
...
//! references. Lenient readers (including xberg-native-pdf's own) tolerate this
//! silently; strict validators reject it. The same defect class was
//! independently diagnosed against `add_text()` +
...
fn set_text_replacement_is_persisted_and_readable() {
    ...
    let (_dir, path) = common::write_temp_pdf(&out, "dom_edit_replace.pdf");
    let doc = PdfDocument::open(&path).unwrap();
    let text = doc.extract_text(0).unwrap();
...
fn set_text_replacement_on_multi_stream_page_leaves_no_dangling_contents_ref() {
    ...
    // that exposed the /Contents merge bug. ~keep
    ...
    let (_dir, path) = common::write_temp_pdf(&out, "multi_stream_contents_replace.pdf");
    let doc = PdfDocument::open(&path).unwrap();
    let text = doc.extract_text(0).unwrap();
