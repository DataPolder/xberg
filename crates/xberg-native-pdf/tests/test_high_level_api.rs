...
use xberg_native_pdf::api::{Pdf, PdfBuilder, PdfConfig};
...
/// Regression guards for Markdown → PDF dropping all styling
...
mod markdown_styling_regression {
    ...
    fn alpha_size_extent(chars: &[xberg_native_pdf::layout::TextChar]) -> (f32, f32) {
        let mut min = f32::MAX;
        let mut max = 0.0_f32;
    ...
    fn heading_is_scaled_and_markers_consumed() {
        // The exact reproduction from the original report. ~keep
    ...
    fn word_chars<'a>(
        chars: &'a [xberg_native_pdf::layout::TextChar],
        word: &str,
    ) -> Vec<&'a xberg_native_pdf::layout::TextChar> {
    ...
    /// the documented trade-off; this guards it.
    #[test]
    ...
    fn snake_case_underscores_are_preserved() {
        ...
        // the old code stripped them, mangling identifiers. ~keep
...
/// collapsed it to plain `Helvetica` — the same root cause as the
/// Markdown styling loss above, via a different public API, previously
/// untested. Guards the latent fix.
mod document_builder_rich_text_regression {
    use xberg_native_pdf::writer::{DocumentBuilder, PageSize, TextRun};
    ...
    fn rich_paragraph_bold_and_italic_select_styled_faces() {
        ...
        let mut pdf = xberg_native_pdf::api::Pdf::from_bytes(bytes).expect("load");
        let chars = pdf.extract_chars(0).expect("extract chars");
    ...
    fn rich_paragraph_inserts_space_between_runs() {
        ...
        let bytes = b.build().expect("build");
        let doc = xberg_native_pdf::document::PdfDocument::from_bytes(bytes).expect("load");
