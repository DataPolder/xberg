...
async fn should_emit_processing_warning_when_citation_parsing_fails() {
    // A citation record with a type but no title: `biblib`'s RIS parser tolerates
    // unrecognized lines by ignoring them, so a genuine `ParseError` requires a
    // record that reaches title extraction and fails a required-field check.
    let malformed_content = b"TY  - JOUR\nAU  - Smith, John\nER  -";

    let config = ExtractionConfig::default();
    let extraction = extract_bytes_document(malformed_content, RIS_MIME, &config)
        .await
        .expect("malformed citation extraction should still succeed with a fallback");

    assert!(
        extraction.content.contains("TY  - JOUR\nAU  - Smith, John\nER  -"),
