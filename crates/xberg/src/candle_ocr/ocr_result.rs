...
fn effective_languages(config: &OcrConfig) -> Vec<String> {
    let langs: Vec<String> = config
        ...
        .map(str::to_string)
        .collect();
    if langs.is_empty() { vec![DEFAULT_LANGUAGE.to_string()] } else { langs }
}
...
mod tests {
    ...
    fn should_build_ocr_document_with_metadata_tables_and_languages() {
        ...
        let png_1x1: &[u8] = &[
            ...
            0x00, 0x01, 0x18, 0xDD, 0x8D, 0xB0, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60,
            0x82,
        ];
        let content = "| H |\n|---|\n| v |".to_string();
        ...
        let doc = build_ocr_document(content.clone(), Vec::new(), Cow::Borrowed("text/markdown"), png_1x1, &config);
