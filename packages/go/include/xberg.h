 ...
 * (BM25/TF-IDF), though â every chunk under the same section repeats the
 * same heading tokens, so a heading word's document frequency approaches the
 * number of chunks in that section and its IDF collapses toward zero, drowning
 * out real match signal.
 *
 * `Metadata` keeps `content` free of the breadcrumb and relies solely on
 * `ChunkMetadata::heading_path` (crate::types::ChunkMetadata::heading_path)
 * (already populated for every chunk under a heading by `chunk_for_rag`
 * (crate::chunking::chunk_for_rag)), which downstream lexical/hybrid pipelines
 * can use to exclude or down-weight heading tokens directly â instead of
 * the fragile pattern of string-stripping a breadcrumb prefix back out of
 * `content`.
 *
 ...
 * precisely what `Content` already does â a third variant would be a
 * synonym, and one that every binding in 15 languages would have to carry and
 * every caller would have to guess the meaning of.
 */
