...
#define XBERG_FEATURE_CLASSIFICATION 1
#define XBERG_FEATURE_DEFAULT 1
#define XBERG_FEATURE_DIFF 1
#define XBERG_FEATURE_EMAIL 1
#define XBERG_FEATURE_EMBEDDINGS 1
#define XBERG_FEATURE_EMBEDDING_PRESETS 1
#define XBERG_FEATURE_ENRICHMENT 1
#define XBERG_FEATURE_EXCEL 1
#define XBERG_FEATURE_FULL_NO_HEIC 1
#define XBERG_FEATURE_HEURISTICS 1
#define XBERG_FEATURE_HTML 1
#define XBERG_FEATURE_HWP 1
#define XBERG_FEATURE_HWPX 1
#define XBERG_FEATURE_IWORK 1
#define XBERG_FEATURE_KEYWORDS_RAKE 1
#define XBERG_FEATURE_KEYWORDS_YAKE 1
#define XBERG_FEATURE_LANGUAGE_DETECTION 1
#define XBERG_FEATURE_LATE_INTERACTION 1
#define XBERG_FEATURE_LATE_INTERACTION_PRESETS 1
#define XBERG_FEATURE_LAYOUT_DETECTION 1
#define XBERG_FEATURE_LAYOUT_TYPES 1
...
#define XBERG_FEATURE_OCR_WASM 1
...
#define XBERG_FEATURE_WORDPERFECT 1
#define XBERG_FEATURE_XML 1
 ...
 * is rescaled into the same 0-100 range but is *not* comparable â its
 ...
 * assuming.
 */
typedef struct XBERGConfidenceSemantics XBERGConfidenceSemantics;
/**
 ...
 */
typedef struct XBERGContentFilterKind XBERGContentFilterKind;
/**
 ...
 */
typedef struct XBERGCrawlStrategyKind XBERGCrawlStrategyKind;
/**
 ...
 *
 * // Table detection is off by default (unlike `TesseractConfig`); PaddleOCR
 * has no
 * // per-word table-candidate signal, so unfiltered clustering over-fabricates
 * tables
 * // on ordinary prose. Enable it explicitly once you've validated it for your
 * corpus. let config = PaddleOcrConfig::new("en") .with_table_detection(true);
 * \endcode
 */
 ...
 * RequiresUpright` and otherwise does nothing. Every other mention forwards the
 * value to that test. So, *to that codepath*, `SelfCorrecting` and
 * `RecognisesRotatedText` are behaviourally identical â the enum is a
 * boolean at the point of use, and the three variants describe measured backend
 * behaviour rather than three dispatch paths.
 *
 * `RecognisesRotatedText`'s actual remedy is not this enum. The block-order fix
 * is the `backend_options["page_rotation_degrees"]` hint injected by
 ...
 * that class must also read the hint. Conversely, gating that hint on this enum
 * would remove a field from Tesseract's `OcrConfig` and hence from the OCR
 * cache key
 * (`hash(image + language + config)`), invalidating every cached page â do
 * not do it without its own A/B.
 *
 * # PDF-route-only
 *
 ...
 */
typedef struct XBERGPageOrientationHandling XBERGPageOrientationHandling;
/**
 ...
 */
typedef struct XBERGPdfBackend XBERGPdfBackend;
/**
 ...
 * crate::ocr::types::TesseractConfig` impl in `ocr/types.rs` â that
 * conversion carries field *values* across, but each struct keeps its own
 * `Default` impl, and the conversion does nothing to keep those two defaults in
 * sync. Several production call sites
 ...
 * convert it, bypassing the internal struct's `Default` entirely â so if
...
enum XBERGAlefFfiErrorCode
#if __STDC_VERSION__ >= 202311L
    : int32_t
#endif // __STDC_VERSION__ >= 202311L
{
  XbergAlefNone = 0,
  XbergAlefConversion = 1,
  XbergAlefUnknown = 2,
  XbergAlefPanic = 3,
...
#else
typedef int32_t XBERGAlefFfiErrorCode;
#endif // __STDC_VERSION__ >= 202311L
...
typedef struct XBERGXbergOcrBackendVTable {
  ...
  /**
   * Declare how this backend's reported page-level confidence must be
   * interpreted.
   *
   * Defaults to [`ConfidenceSemantics::Uncalibrated`]. This default is
   * deliberately the least trusting option, not
   * [`ConfidenceSemantics::Legibility`]: a new backend that reports *some*
   * confidence number is not thereby safe to gate on, and defaulting to
   * `Legibility` would let the next backend silently inherit a threshold
   * calibrated for a different backend's scale — exactly the failure this type
   * exists to prevent (see the type's doc comment). Override this only after
   * validating that the reported number tracks legibility on a known scale.
   */
  int32_t (*confidence_semantics)(const void *user_data, char **out_result,
                                  char **out_error);
  /**
   * Declare how this backend copes with a page raster whose text is not
   * upright.
   *
   * Defaults to [`PageOrientationHandling::RequiresUpright`]. This default is
   * deliberately the least capable option, not
   * [`PageOrientationHandling::SelfCorrecting`]: a new backend must not
   * silently inherit Tesseract's ability to reconstruct reading order on a
   * rotated raster and then quietly emit garbage the first time it faces one
   * (see the type's doc comment for the measured A/B behind this). Override
   * this only after validating the backend's actual behaviour on a rotated
   * page.
   */
  int32_t (*page_orientation_handling)(const void *user_data, char **out_result,
                                       char **out_error);
  /**
 ...
 */
uintptr_t
xberg_chunk_classification_config_max_concurrency(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_content_filter_config_strip_repeating_text(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_content_filter_config_default(void);
 ...
 */
int32_t
xberg_extraction_config_enable_quality_processing(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_extraction_config_language_detection(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_extraction_config_include_document_structure(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_extraction_config_structured_extraction(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_extraction_config_page_classification(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_extraction_config_chunk_classification(XBERGAlefHandle handle);
 ...
 * while giving legitimate but slow paths â VLM-based OCR, large scanned
 * documents â enough headroom to finish instead of being cut off at the
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_extraction_config_default(void);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_ocr_strategy(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_content_filter(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_pdf_options(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_token_reduction(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_language_detection(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_html_output(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_result_format(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_output_format(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_transcription(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_tree_sitter(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_structured_extraction(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_summarization(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_translation(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_page_classification(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_file_extraction_config_chunk_classification(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_svg_options_default(void);
#endif
 ...
 * Pointer must be a valid handle returned by this library.
 */
uint8_t *xberg_extract_input_bytes(XBERGAlefHandle handle, uintptr_t *out_len);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_extract_input_default(void);
 ...
 */
char *
xberg_extraction_result_crawl_unique_normalized_urls(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_url_extraction_config_allow_local_file_inputs(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_url_extraction_config_default(void);

#if (defined(XBERG_FEATURE_URL_INGESTION) ||                                   \
     defined(XBERG_FEATURE_URL_CONFIG_TYPES))
/**
 ...
 */
int32_t
xberg_image_extraction_config_max_image_dimension(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_image_extraction_config_inject_placeholders(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_image_extraction_config_include_page_rasters(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_image_extraction_config_output_format(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_image_extraction_config_include_data_base64(XBERGAlefHandle handle);

/**
 * \note SAFETY: Caller must ensure all pointer arguments are valid or null.
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_image_extraction_config_default(void);
 ...
 */
int32_t
xberg_token_reduction_options_preserve_important_words(XBERGAlefHandle handle);

/**
 * \note SAFETY: Caller must ensure all pointer arguments are valid or null.
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_token_reduction_options_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_language_detection_config_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_html_output_config_default(void);
#endif
 ...
 */
uintptr_t
xberg_late_interaction_config_query_max_length(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_late_interaction_config_show_download_progress(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_late_interaction_config_acceleration(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_layout_detection_config_table_model(XBERGAlefHandle handle);
#endif
 ...
 */
XBERGAlefHandle
xberg_layout_detection_config_formula_model(XBERGAlefHandle handle);
#endif
 ...
 */
XBERGAlefHandle
xberg_layout_detection_config_table_overlap_preference(XBERGAlefHandle handle);
#endif
 ...
 */
XBERGAlefHandle
xberg_layout_detection_config_acceleration(XBERGAlefHandle handle);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_layout_detection_config_default(void);
#endif
...
/**
 * Get the `max_concurrency` field from a `LlmConfig`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
 ...
 */
char *
xberg_structured_extraction_config_schema_description(XBERGAlefHandle handle);
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_min_total_non_whitespace(XBERGAlefHandle handle);
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_min_meaningful_word_len(XBERGAlefHandle handle);
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_min_meaningful_words(XBERGAlefHandle handle);
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_min_garbage_chars(XBERGAlefHandle handle);
 ...
 */
double
xberg_ocr_quality_thresholds_max_fragmented_word_ratio(XBERGAlefHandle handle);
...
/**
 * Get the `max_ocr_output_fragmented_word_ratio` field from a
 * `OcrQualityThresholds`. # Safety Pointer must be a valid handle returned by
 * this library.
 */
double xberg_ocr_quality_thresholds_max_ocr_output_fragmented_word_ratio(
    XBERGAlefHandle handle);

/**
 * Get the `min_ocr_mean_confidence` field from a `OcrQualityThresholds`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
double
xberg_ocr_quality_thresholds_min_ocr_mean_confidence(XBERGAlefHandle handle);

/**
 * Get the `min_words_for_ocr_output_check` field from a `OcrQualityThresholds`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
uintptr_t xberg_ocr_quality_thresholds_min_words_for_ocr_output_check(
    XBERGAlefHandle handle);

/**
 * Get the `max_ocr_output_dict_invalid_word_ratio` field from a
 * `OcrQualityThresholds`. # Safety Pointer must be a valid handle returned by
 * this library.
 */
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_min_words_for_repeat_check(XBERGAlefHandle handle);
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_substantive_min_chars(XBERGAlefHandle handle);
 ...
 */
uintptr_t
xberg_ocr_quality_thresholds_non_text_min_chars(XBERGAlefHandle handle);
 ...
 */
double
xberg_ocr_quality_thresholds_alnum_ws_ratio_threshold(XBERGAlefHandle handle);
 ...
 */
double
xberg_ocr_quality_thresholds_pipeline_min_quality(XBERGAlefHandle handle);
 ...
 */
double
xberg_ocr_quality_thresholds_min_undecodable_ratio(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_ocr_quality_thresholds_default(void);
 ...
 */
XBERGAlefHandle
xberg_ocr_pipeline_stage_tesseract_config(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_ocr_pipeline_config_quality_thresholds(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_ocr_config_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_page_config_default(void);
 ...
 * A non-null returned handle is owned by the caller.
 * It must be freed with `xberg_pdf_backend_free`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
XBERGAlefHandle xberg_pdf_config_backend(XBERGAlefHandle handle);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_hierarchy_config_default(void);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_post_processor_config_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_chunking_config_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_embedding_config_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_redaction_config_default(void);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_reranker_config_default(void);
 ...
 */
int32_t
xberg_sparse_embedding_config_show_download_progress(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_sparse_embedding_config_acceleration(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_transcription_config_default(void);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_tree_sitter_config_default(void);
#endif
 ...
 */
int32_t
xberg_tree_sitter_process_config_data_extraction(XBERGAlefHandle handle);
#endif
 ...
 */
uintptr_t
xberg_tree_sitter_process_config_chunk_max_size(XBERGAlefHandle handle);
#endif
 ...
 */
XBERGAlefHandle
xberg_tree_sitter_process_config_content_mode(XBERGAlefHandle handle);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_server_config_default(void);
#endif
 ...
 */
int32_t
xberg_docx_app_properties_characters_with_spaces(XBERGAlefHandle handle);
#endif
...
/**
 * Get the `max_pages` field from a `SecurityLimits`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_security_limits_default(void);
 ...
 */
int32_t
xberg_token_reduction_config_enable_semantic_clustering(XBERGAlefHandle handle);
#endif
 ...
 */
int32_t
xberg_token_reduction_config_preserve_important_words(XBERGAlefHandle handle);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_token_reduction_config_default(void);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_footnote_config_default(void);
#endif
 ...
 */
XBERGAlefHandle
xberg_footnote_config_with_parse_citations(XBERGAlefHandle this_,
                                           int32_t enabled);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_document_structure_default(void);
 ...
 */
XBERGAlefHandle
xberg_extracted_document_extraction_method(XBERGAlefHandle handle);
 ...
 */
char *
xberg_extracted_document_detected_language_confidences(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_extracted_document_extraction_confidence(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_extracted_document_redaction_report(XBERGAlefHandle handle);
 ...
 * Pointer must be a valid handle returned by this library.
 */
uint8_t *xberg_extracted_image_data(XBERGAlefHandle handle, uintptr_t *out_len);
 ...
 */
XBERGAlefHandle
xberg_pptx_extraction_result_page_structure(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_image_preprocessing_config_contrast_enhance(XBERGAlefHandle handle);
 ...
 */
char *
xberg_image_preprocessing_config_binarization_method(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_image_preprocessing_config_default(void);
 ...
 */
int32_t
xberg_tesseract_config_tessedit_dont_blkrej_good_wds(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_tesseract_config_tessedit_dont_rowrej_good_wds(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_tesseract_config_tessedit_enable_dict_correction(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_tesseract_config_textord_space_size_is_variable(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_tesseract_config_default(void);
 ...
 */
int32_t
xberg_image_preprocessing_metadata_auto_adjusted(XBERGAlefHandle handle);
 ...
 */
char *
xberg_image_preprocessing_metadata_resample_method(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_image_preprocessing_metadata_dimension_clamped(XBERGAlefHandle handle);
 ...
 */
int32_t
xberg_image_preprocessing_metadata_skipped_resize(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_yake_params_default(void);
#endif

#if defined(XBERG_FEATURE_KEYWORDS_RAKE)
/**
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_rake_params_default(void);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_keyword_config_default(void);
#endif
 ...
 */
XBERGAlefHandle
xberg_document_metadata_user_chunk_config(XBERGAlefHandle handle);
#endif
 ...
 */
XBERGAlefHandle
xberg_extraction_confidence_schema_compliance(XBERGAlefHandle handle);
#endif
 ...
 */
int32_t
xberg_heuristics_config_enable_pdf_text_heuristics(XBERGAlefHandle handle);
#endif
 ...
 */
uint64_t
xberg_heuristics_config_file_size_threshold_bytes(XBERGAlefHandle handle);
#endif
 ...
 */
uint64_t
xberg_heuristics_config_disk_processing_threshold_bytes(XBERGAlefHandle handle);
#endif
 ...
 */
uint64_t
xberg_heuristics_config_max_xlsx_workbook_cells(XBERGAlefHandle handle);
#endif
 ...
 */
uint32_t
xberg_heuristics_config_max_pptx_embedded_count(XBERGAlefHandle handle);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_heuristics_config_default(void);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_multidoc_thresholds_default(void);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_registry_get(XBERGAlefHandle this_, const char *id);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
int32_t xberg_registry_sample_bytes(XBERGAlefHandle this_,
 ...
 */
XBERGAlefHandle
xberg_resolved_preset_preferred_call_mode(XBERGAlefHandle handle);
#endif
 ...
 */
XBERGAlefHandle
xberg_preset_summary_preferred_call_mode(XBERGAlefHandle handle);
#endif
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_doctor_check_pass(const char *name, const char *message);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_doctor_check_warn(const char *name, const char *message);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_doctor_check_fail(const char *name, const char *message);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
XBERGAlefHandle xberg_doctor_check_skip(const char *name, const char *message);
...
#if defined(XBERG_FEATURE_AUTO_ROTATE_TYPES)
/**
 ...
 * Pointer must be a valid handle returned by this library.
 */
uint8_t *xberg_embedded_file_data(XBERGAlefHandle handle, uintptr_t *out_len);
#endif
...
#if defined(XBERG_FEATURE_CLASSIFICATION)
/**
 ...
 */
XBERGAlefHandle
xberg_chunk_classification_enrichment_config_config(XBERGAlefHandle handle);
#endif
 ...
 * A non-null returned handle is owned by the caller.
 * It must be freed with `xberg_crawl_strategy_kind_free`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
XBERGAlefHandle xberg_crawl_config_crawl_strategy(XBERGAlefHandle handle);

/**
 * Get the `content_filter` field from a `CrawlConfig`.
 * A non-null returned handle is owned by the caller.
 * It must be freed with `xberg_content_filter_kind_free`.
 * # Safety
 * Pointer must be a valid handle returned by this library.
 */
XBERGAlefHandle xberg_crawl_config_content_filter(XBERGAlefHandle handle);
 ...
 * Pointer must be a valid handle returned by this library.
 */
char *xberg_crawl_config_bm25_query(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_crawl_config_document_content_encoding(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_conversion_options_list_indent_type(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_conversion_options_highlight_style(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_conversion_options_whitespace_mode(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_conversion_options_code_block_style(XBERGAlefHandle handle);
 ...
 */
XBERGAlefHandle
xberg_conversion_options_url_escape_style(XBERGAlefHandle handle);
...
/**
 * Convert a `FormulaModel` serde wire value (C string) to its integer
 * discriminant. Returns -1 on invalid input. # Safety Caller must ensure `ptr`
 * is a valid pointer to a `c_char` or null.
 */
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
int32_t xberg_pdf_backend_from_i32(int32_t value);

/**
 * Convert a `PdfBackend` serde wire value (C string) to its integer
 * discriminant. Returns -1 on invalid input. # Safety Caller must ensure `ptr`
 * is a valid pointer to a `c_char` or null.
 */
int32_t xberg_pdf_backend_from_str(const char *name);
...
/**
 * Convert an integer to a `ConfidenceSemantics` variant. Returns -1 on invalid
 * input. # Safety Caller must ensure all pointer arguments are valid or null.
 * Returned pointers must be freed with the appropriate free function.
 */
int32_t xberg_confidence_semantics_from_i32(int32_t value);

/**
 * Convert a `ConfidenceSemantics` serde wire value (C string) to its integer
 * discriminant. Returns -1 on invalid input. # Safety Caller must ensure `ptr`
 * is a valid pointer to a `c_char` or null.
 */
int32_t xberg_confidence_semantics_from_str(const char *name);
 ...
 * null. Returned pointers must be freed with the appropriate free function.
 */
int32_t xberg_page_orientation_handling_from_i32(int32_t value);

/**
 * Convert a `PageOrientationHandling` serde wire value (C string) to its
 * integer discriminant. Returns -1 on invalid input. # Safety Caller must
 * ensure `ptr` is a valid pointer to a `c_char` or null.
 */
int32_t xberg_page_orientation_handling_from_str(const char *name);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
int32_t xberg_crawl_strategy_kind_from_i32(int32_t value);
 ...
 * is a valid pointer to a `c_char` or null.
 */
int32_t xberg_crawl_strategy_kind_from_str(const char *name);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
int32_t xberg_content_filter_kind_from_i32(int32_t value);
 ...
 * is a valid pointer to a `c_char` or null.
 */
int32_t xberg_content_filter_kind_from_str(const char *name);
...
/**
 * Free a `ExecutionProviderType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_execution_provider_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `ExecutionProviderType` to a JSON string. Returns null on
 * failure. # Safety `handle` must be a valid, non-zero handle returned by a
 * `xberg` function. The returned string must be freed with `xberg_free_string`.
 */
char *xberg_execution_provider_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `ExecutionProviderType` as its string representation
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_execution_provider_type_to_string(XBERGAlefHandle handle);

/**
 * Free a `ImageOutputFormat` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_image_output_format_free(XBERGAlefHandle handle);

/**
 * Serialize a `ImageOutputFormat` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_image_output_format_to_json(XBERGAlefHandle handle);

/**
 * Render a `ImageOutputFormat` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_image_output_format_to_string(XBERGAlefHandle handle);

/**
 * Free a `ExtractInputKind` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_extract_input_kind_free(XBERGAlefHandle handle);

/**
 * Serialize a `ExtractInputKind` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_extract_input_kind_to_json(XBERGAlefHandle handle);

/**
 * Render a `ExtractInputKind` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_extract_input_kind_to_string(XBERGAlefHandle handle);

/**
 * Free a `UrlExtractionMode` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_url_extraction_mode_free(XBERGAlefHandle handle);

/**
 * Serialize a `UrlExtractionMode` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_url_extraction_mode_to_json(XBERGAlefHandle handle);

/**
 * Render a `UrlExtractionMode` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_url_extraction_mode_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_output_format_free(XBERGAlefHandle handle);

/**
 * Serialize a `OutputFormat` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_output_format_to_json(XBERGAlefHandle handle);

/**
 * Render a `OutputFormat` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_jupyter_cell_rendering_free(XBERGAlefHandle handle);

/**
 * Serialize a `JupyterCellRendering` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_jupyter_cell_rendering_to_json(XBERGAlefHandle handle);

/**
 * Render a `JupyterCellRendering` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_late_interaction_model_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `LateInteractionModelType` to a JSON string. Returns null on
 * failure. # Safety `handle` must be a valid, non-zero handle returned by a
 * `xberg` function. The returned string must be freed with `xberg_free_string`.
 */
char *xberg_late_interaction_model_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `LateInteractionModelType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
...
/**
 * Free a `CredentialProviderConfig` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_credential_provider_config_free(XBERGAlefHandle handle);

/**
 * Serialize a `CredentialProviderConfig` to a JSON string. Returns null on
 * failure. # Safety `handle` must be a valid, non-zero handle returned by a
 * `xberg` function. The returned string must be freed with `xberg_free_string`.
 */
char *xberg_credential_provider_config_to_json(XBERGAlefHandle handle);

/**
 * Render a `CredentialProviderConfig` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_credential_provider_config_to_string(XBERGAlefHandle handle);

/**
 * Free a `CallMode` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_call_mode_free(XBERGAlefHandle handle);

/**
 * Serialize a `CallMode` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_call_mode_to_json(XBERGAlefHandle handle);

/**
 * Render a `CallMode` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
...
/**
 * Free a `MergeMode` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_merge_mode_free(XBERGAlefHandle handle);

/**
 * Serialize a `MergeMode` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_merge_mode_to_json(XBERGAlefHandle handle);

/**
 * Render a `MergeMode` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_ner_backend_kind_free(XBERGAlefHandle handle);

/**
 * Serialize a `NerBackendKind` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ner_backend_kind_to_json(XBERGAlefHandle handle);

/**
 * Render a `NerBackendKind` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ner_backend_kind_to_string(XBERGAlefHandle handle);

/**
 * Free a `VlmFallbackPolicy` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_vlm_fallback_policy_free(XBERGAlefHandle handle);

/**
 * Serialize a `VlmFallbackPolicy` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_vlm_fallback_policy_to_json(XBERGAlefHandle handle);

/**
 * Render a `VlmFallbackPolicy` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
...
/**
 * Free a `OcrStrategy` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_ocr_strategy_free(XBERGAlefHandle handle);

/**
 * Serialize a `OcrStrategy` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ocr_strategy_to_json(XBERGAlefHandle handle);

/**
 * Render a `OcrStrategy` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ocr_strategy_to_string(XBERGAlefHandle handle);
...
/**
 * Free a `TableChunkingMode` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_table_chunking_mode_free(XBERGAlefHandle handle);

/**
 * Serialize a `TableChunkingMode` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_table_chunking_mode_to_json(XBERGAlefHandle handle);

/**
 * Render a `TableChunkingMode` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
...
/**
 * Free a `ChunkerType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_chunker_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `ChunkerType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_chunker_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `ChunkerType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_chunker_type_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_chunk_sizing_free(XBERGAlefHandle handle);

/**
 * Serialize a `ChunkSizing` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_chunk_sizing_to_json(XBERGAlefHandle handle);

/**
 * Render a `ChunkSizing` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_chunk_sizing_to_string(XBERGAlefHandle handle);

/**
 * Free a `EmbeddingModelType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_embedding_model_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `EmbeddingModelType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_embedding_model_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `EmbeddingModelType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_embedding_model_type_to_string(XBERGAlefHandle handle);

/**
 * Free a `RerankerModelType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_reranker_model_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `RerankerModelType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_reranker_model_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `RerankerModelType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_reranker_model_type_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_sparse_embedding_model_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `SparseEmbeddingModelType` to a JSON string. Returns null on
 * failure. # Safety `handle` must be a valid, non-zero handle returned by a
 * `xberg` function. The returned string must be freed with `xberg_free_string`.
 */
char *xberg_sparse_embedding_model_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `SparseEmbeddingModelType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_sparse_embedding_model_type_to_string(XBERGAlefHandle handle);
...
/**
 * Free a `PdfAnnotationType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_pdf_annotation_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `PdfAnnotationType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_pdf_annotation_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `PdfAnnotationType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
...
/**
 * Free a `BlockType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_block_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `BlockType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_block_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `BlockType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_inline_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `InlineType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_inline_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `InlineType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_inline_type_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_relationship_kind_free(XBERGAlefHandle handle);

/**
 * Serialize a `RelationshipKind` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_relationship_kind_to_json(XBERGAlefHandle handle);

/**
 * Render a `RelationshipKind` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_relationship_kind_to_string(XBERGAlefHandle handle);

/**
 * Free a `ContentLayer` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_content_layer_free(XBERGAlefHandle handle);

/**
 * Serialize a `ContentLayer` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_content_layer_to_json(XBERGAlefHandle handle);

/**
 * Render a `ContentLayer` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_content_layer_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_node_content_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_node_content_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_node_content_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_annotation_kind_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_annotation_kind_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_annotation_kind_to_string(XBERGAlefHandle handle);

/**
 * Free a `EntityCategory` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_entity_category_free(XBERGAlefHandle handle);

/**
 * Serialize a `EntityCategory` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_entity_category_to_json(XBERGAlefHandle handle);

/**
 * Render a `EntityCategory` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_entity_category_to_string(XBERGAlefHandle handle);

/**
 * Free a `ExtractionMethod` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_extraction_method_free(XBERGAlefHandle handle);

/**
 * Serialize a `ExtractionMethod` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_extraction_method_to_json(XBERGAlefHandle handle);

/**
 * Render a `ExtractionMethod` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_extraction_method_to_string(XBERGAlefHandle handle);

/**
 * Free a `ChunkType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_chunk_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `ChunkType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_chunk_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `ChunkType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_chunk_type_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_image_kind_free(XBERGAlefHandle handle);

/**
 * Serialize a `ImageKind` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_image_kind_to_json(XBERGAlefHandle handle);

/**
 * Render a `ImageKind` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_image_kind_to_string(XBERGAlefHandle handle);

/**
 * Free a `ResultFormat` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_result_format_free(XBERGAlefHandle handle);

/**
 * Serialize a `ResultFormat` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_result_format_to_json(XBERGAlefHandle handle);

/**
 * Render a `ResultFormat` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_result_format_to_string(XBERGAlefHandle handle);

/**
 * Free a `ElementType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_element_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `ElementType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_element_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `ElementType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_element_type_to_string(XBERGAlefHandle handle);

/**
 * Free a `FormFieldType` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_form_field_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `FormFieldType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_form_field_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `FormFieldType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_form_field_type_to_string(XBERGAlefHandle handle);

/**
 * Free a `FormatMetadata` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_format_metadata_free(XBERGAlefHandle handle);

/**
 * Serialize a `FormatMetadata` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_format_metadata_to_json(XBERGAlefHandle handle);

/**
 * Render a `FormatMetadata` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_text_direction_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_text_direction_to_json(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_link_type_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_link_type_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_link_type_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_image_type_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_image_type_to_json(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_structured_data_type_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_structured_data_type_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_structured_data_type_to_string(XBERGAlefHandle handle);

/**
 * Free a `OcrBoundingGeometry` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_ocr_bounding_geometry_free(XBERGAlefHandle handle);

/**
 * Serialize a `OcrBoundingGeometry` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ocr_bounding_geometry_to_json(XBERGAlefHandle handle);

/**
 * Render a `OcrBoundingGeometry` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ocr_bounding_geometry_to_string(XBERGAlefHandle handle);

/**
 * Free a `OcrElementLevel` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_ocr_element_level_free(XBERGAlefHandle handle);

/**
 * Serialize a `OcrElementLevel` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ocr_element_level_to_json(XBERGAlefHandle handle);

/**
 * Render a `OcrElementLevel` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_ocr_element_level_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_page_unit_type_free(XBERGAlefHandle handle);

/**
 * Serialize a `PageUnitType` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_page_unit_type_to_json(XBERGAlefHandle handle);

/**
 * Render a `PageUnitType` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_page_unit_type_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_redaction_strategy_free(XBERGAlefHandle handle);

/**
 * Serialize a `RedactionStrategy` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_redaction_strategy_to_json(XBERGAlefHandle handle);

/**
 * Render a `RedactionStrategy` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_pii_category_free(XBERGAlefHandle handle);

/**
 * Serialize a `PiiCategory` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_pii_category_to_json(XBERGAlefHandle handle);

/**
 * Render a `PiiCategory` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_pii_category_to_string(XBERGAlefHandle handle);

/**
 * Free a `RevisionKind` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_revision_kind_free(XBERGAlefHandle handle);

/**
 * Serialize a `RevisionKind` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_revision_kind_to_json(XBERGAlefHandle handle);

/**
 * Render a `RevisionKind` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
...
/**
 * Free a `RevisionAnchor` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_revision_anchor_free(XBERGAlefHandle handle);

/**
 * Serialize a `RevisionAnchor` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_revision_anchor_to_json(XBERGAlefHandle handle);

/**
 * Render a `RevisionAnchor` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_revision_anchor_to_string(XBERGAlefHandle handle);

/**
 * Free a `SummaryStrategy` handle.
 * # Safety
 * Handle must have been returned by this library, or be zero.
 */
void xberg_summary_strategy_free(XBERGAlefHandle handle);

/**
 * Serialize a `SummaryStrategy` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_summary_strategy_to_json(XBERGAlefHandle handle);

/**
 * Render a `SummaryStrategy` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_summary_strategy_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_uri_kind_free(XBERGAlefHandle handle);

/**
 * Serialize a `UriKind` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_uri_kind_to_json(XBERGAlefHandle handle);

/**
 * Render a `UriKind` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_uri_kind_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_probe_status_free(XBERGAlefHandle handle);

/**
 * Serialize a `ProbeStatus` to a JSON string. Returns null on failure.
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_probe_status_to_json(XBERGAlefHandle handle);

/**
 * Render a `ProbeStatus` as its string representation
 * (the unit-variant name as serialized by serde — e.g. `"completed"`,
 * without surrounding JSON quotes).
 * # Safety
 * `handle` must be a valid, non-zero handle returned by a `xberg` function.
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_browser_mode_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_browser_mode_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_browser_mode_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_browser_wait_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_browser_wait_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_browser_wait_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_browser_backend_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_browser_backend_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_browser_backend_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_document_content_encoding_free(XBERGAlefHandle handle);
 ...
 * `xberg` function. The returned string must be freed with `xberg_free_string`.
 */
char *xberg_document_content_encoding_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_document_content_encoding_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_crawl_strategy_kind_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_crawl_strategy_kind_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_crawl_strategy_kind_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_content_filter_kind_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_content_filter_kind_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_content_filter_kind_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_auth_config_free(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_auth_config_to_json(XBERGAlefHandle handle);
 ...
 * The returned string must be freed with `xberg_free_string`.
 */
char *xberg_auth_config_to_string(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_heading_style_free(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_list_indent_type_free(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_newline_style_free(XBERGAlefHandle handle);
 ...
 * Handle must have been returned by this library, or be zero.
 */
void xberg_link_style_free(XBERGAlefHandle handle);
 ...
 * Returned pointers must be freed with the appropriate free function.
 */
char *xberg_max_sim_rank(XBERGAlefHandle query, const char *docs);
#endif
