...
public sealed record LlmConfig {
  ...
  /// entirely on that target, via the crate-root `#[cfg(all(feature =
  /// "liter-llm", not(target_arch = "wasm32")))] pub mod llm;` gate in
  /// `lib.rs`. Every variant needs liter-llm's `native-http`-backed auth
  /// modules, and wasm32 builds request only `wasm-http` (see the `liter-llm`
  /// dependency comment in Cargo.toml), so there is no code path left on that
  /// target to construct a provider from this field, or to reject it. This type
  /// (`core.config.llm`) has no `liter-llm` dependency itself and compiles on
  /// every target, so setting this field on a wasm32 build is accepted by serde
  /// and silently ignored — a plain no-op, not a `Validation`. Reject a wasm32
  /// build that sets this field yourself if that silence is a problem for your
  /// use case; xberg does not do it for you.
  ///
