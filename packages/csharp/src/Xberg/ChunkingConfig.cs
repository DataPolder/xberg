...
public sealed record ChunkingConfig {
  ...
  /// the full rationale. `heading_context`/`heading_path` on `ChunkMetadata`
  /// are populated independently of this flag, so callers lose no information —
  /// only the in-place mutation is gone.
  ///
  /// Call `render_heading_breadcrumb`
  /// explicitly at index time instead, for the retrieval consumer that wants
  /// the breadcrumb inline.
  ///
