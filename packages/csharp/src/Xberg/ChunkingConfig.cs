...
public sealed record ChunkingConfig {
  ...
  /// <summary>
  /// When `true` and `chunker_type` is `Markdown`, prepend the heading
  /// hierarchy path (e.g. `"# Title &gt; ## Section\n\n"`) to each chunk's
  /// content string.
  ///
  ...
  public bool PrependHeadingContext {
    get; init;
  } = false;
  ...
  /// `prepend_heading_context` is `true`. See `BreadcrumbTarget` for the
  /// dense-vs-lexical retrieval trade-off. Has no effect when
  /// `prepend_heading_context` is `false`.
  ///
  /// Default: `Content` — preserves the pre-existing behaviour of prepending
  /// the breadcrumb into chunk `content`.
  /// </summary>
