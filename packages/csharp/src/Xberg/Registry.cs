...
public sealed class Registry : IDisposable {
  ...
  public Preset? Get(string id) {
    var nativeResult = NativeMethods.RegistryGet(Handle, id);
    if (nativeResult == IntPtr.Zero) {
      return null;
    }
  ...
  public List<PresetSummary> Summaries() {
    var nativeResult = NativeMethods.RegistrySummaries(Handle);
    if (nativeResult == IntPtr.Zero) {
      var ec = NativeMethods.LastErrorCode();
      var ctxPtr = NativeMethods.LastErrorContext();
      var msg = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
  ...
  /// </summary>
  public ulong Len() {
    var nativeResult = NativeMethods.RegistryLen(Handle);
  ...
  /// </summary>
  public bool IsEmpty() {
    var nativeResult = NativeMethods.RegistryIsEmpty(Handle);
  ...
  public byte[]? SampleBytes(string presetId, string name) {
    var nativeResult =
        NativeMethods.RegistrySampleBytes(Handle, presetId, name);
    if (nativeResult == IntPtr.Zero) {
      return null;
    }
    var json = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(
  ...
  public ulong ExtendFromDir(string dir) {
    var nativeResult = NativeMethods.RegistryExtendFromDir(Handle, dir);
