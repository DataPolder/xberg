...
# alef:hash:97632e18ac5ffb98a956e7e698fe55ef76dbd6368d7dd1681d468043469b967a
...
module Xberg
  ...
  module NoChunkingReason
    ...
    def self.from_hash(hash)
      discriminator = hash[:type] || hash["type"]
      case discriminator
      when "SmallFile" then NoChunkingReasonSmallFile.from_hash(hash)
      when "FewPages" then NoChunkingReasonFewPages.from_hash(hash)
      when "TextLayerDetected" then NoChunkingReasonTextLayerDetected.from_hash(hash)
      when "FormatNotChunkable" then NoChunkingReasonFormatNotChunkable.from_hash(hash)
      when "ChunkingDisabled" then NoChunkingReasonChunkingDisabled.from_hash(hash)
      when "FastTextExtraction" then NoChunkingReasonFastTextExtraction.from_hash(hash)
      else raise "Unknown discriminator: #{discriminator}"
      end
    end
  end
...
module Xberg
  ...
  module ChunkingReason
    ...
    def self.from_hash(hash)
      discriminator = hash[:type] || hash["type"]
      case discriminator
      when "LargeFile" then ChunkingReasonLargeFile.from_hash(hash)
      when "ManyPages" then ChunkingReasonManyPages.from_hash(hash)
      when "OcrRequired" then ChunkingReasonOcrRequired.from_hash(hash)
      when "LargeAndManyPages" then ChunkingReasonLargeAndManyPages.from_hash(hash)
      else raise "Unknown discriminator: #{discriminator}"
      end
    end
  end
