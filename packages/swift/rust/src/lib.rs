...
mod ffi {
    ...
    extern "Rust" {
        ...
        #[swift_bridge(init)]
        fn new(global_limit: Option<f64>, model_limits: String, enforcement: Option<String>) -> LlmBudgetConfig;
    ...
    extern "Rust" {
        ...
        fn detected_language_confidences(&self) -> Option<Vec<LanguageConfidence>>;
...
impl LlmConfig {
    pub fn new(
        ...
        if let Some(s) = reasoning_effort {
            // Try JSON parse first (handles enum/object values); on parse failure
            // treat the raw input as a JSON string scalar so plain `String` /
            // string-like enum fields don't end up empty for inputs that aren't
            // valid JSON tokens (e.g. `tts-1`).
            let __v = ::serde_json::from_str::<::serde_json::Value>(&s).unwrap_or(::serde_json::Value::String(s));
            if let Ok(t) = ::serde_json::from_value(__v) {
                __target.reasoning_effort = Some(t);
            }
        }
        ...
        if let Some(v) = providers {
            __target.providers = Some(v.into_iter().map(|w| w.0).collect());
        }
        if let Some(w) = cache {
            __target.cache = Some(Box::new(w.0));
        }
        if let Some(w) = budget {
            __target.budget = Some(Box::new(w.0));
        }
        if let Some(w) = rate_limit {
            __target.rate_limit = Some(Box::new(w.0));
        }
        __target.cost_tracking = cost_tracking;
        __target.tracing = tracing;
        __target.cooldown_secs = cooldown_secs;
        __target.health_check_secs = health_check_secs;
        if let Some(w) = bedrock {
            __target.bedrock = Some(Box::new(w.0));
        }
        // alef: credential_provider (CredentialProviderConfig) is an enum; reverse From not generated — left at default
        LlmConfig(__target)
    }
    pub fn model(&self) -> String {
        self.0.model.clone()
    ...
    pub fn providers(&self) -> Option<Vec<LlmProviderConfig>> {
        self.0
            .providers
            .as_ref()
            .map(|v| v.iter().map(|elem| LlmProviderConfig(elem.clone())).collect())
    }
...
impl LlmCacheConfig {
    pub fn new(
        ...
        let mut __target: xberg::core::config::LlmCacheConfig = ::std::default::Default::default();
        ...
        if let Some(s) = backend {
            ...
            let __v = ::serde_json::from_str::<::serde_json::Value>(&s).unwrap_or(::serde_json::Value::String(s));
            if let Ok(t) = ::serde_json::from_value(__v) {
                __target.backend = Some(t);
            }
        }
        if let Ok(v) = ::serde_json::from_str::<::serde_json::Value>(&backend_config) {
            if let Ok(t) = ::serde_json::from_value(v) {
                __target.backend_config = t;
            }
        }
        LlmCacheConfig(__target)
    }
...
impl LlmBudgetConfig {
    pub fn new(global_limit: Option<f64>, model_limits: String, enforcement: Option<String>) -> LlmBudgetConfig {
        let mut __target: xberg::core::config::LlmBudgetConfig = ::std::default::Default::default();
        ...
        if let Some(s) = enforcement {
            ...
            let __v = ::serde_json::from_str::<::serde_json::Value>(&s).unwrap_or(::serde_json::Value::String(s));
            if let Ok(t) = ::serde_json::from_value(__v) {
                __target.enforcement = Some(t);
            }
        }
        LlmBudgetConfig(__target)
    }
...
impl PdfAnnotation {
    ...
    pub fn quad_points(&self) -> String {
        serde_json::to_string(&self.0.quad_points).expect("serializable optional vec")
    }
...
impl ExtractedDocument {
    pub fn new(
        ...
        if let Some(v) = detected_language_confidences {
            __target.detected_language_confidences = Some(v.into_iter().map(|w| w.0).collect());
        }
    ...
    pub fn detected_language_confidences(&self) -> Option<Vec<LanguageConfidence>> {
        self.0
            .detected_language_confidences
            .as_ref()
            .map(|v| v.iter().map(|elem| LanguageConfidence(elem.clone())).collect())
    }
...
impl PdfMetadata {
    pub fn new(
        ...
        if let Ok(__v) = ::serde_json::to_value(layout_gated_pages) {
            if let Ok(t) = ::serde_json::from_value(__v) {
                __target.layout_gated_pages = t;
            }
        }
        if let Ok(__v) = ::serde_json::to_value(layout_gate_reasons) {
            if let Ok(t) = ::serde_json::from_value(__v) {
                __target.layout_gate_reasons = t;
            }
        }
    ...
    pub fn layout_gated_pages(&self) -> Option<Vec<u32>> {
        self.0.layout_gated_pages.as_ref().and_then(|v| {
            ::serde_json::to_value(v)
                .ok()
                .and_then(|j| ::serde_json::from_value(j).ok())
        })
    }
    pub fn layout_gate_reasons(&self) -> Option<Vec<String>> {
        self.0.layout_gate_reasons.as_ref().and_then(|v| {
            ::serde_json::to_value(v)
                .ok()
                .and_then(|j| ::serde_json::from_value(j).ok())
        })
