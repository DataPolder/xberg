...
// alef:hash:fad74b1760ace9f177486b824a0c8278428ead46bc4718e7a92cb02fece3f307
...
internal extension LayoutDetectionConfig {
    init(_ rb: RustBridge.LayoutDetectionConfigRef) throws {
        self.strategy = try { let rawValue = rb.strategy().toString(); guard let value = LayoutStrategy(rawValue: rawValue) else { throw XbergError.validation(message: "Unknown LayoutStrategy variant", source: rawValue) }; return value }()
...
internal extension PdfConfig {
    init(_ rb: RustBridge.PdfConfigRef) throws {
        ...
        self.backend = try { let rawValue = rb.backend().toString(); guard let value = PdfBackend(rawValue: rawValue) else { throw XbergError.validation(message: "Unknown PdfBackend variant", source: rawValue) }; return value }()
    }
...
public struct SecurityLimits: Codable, Sendable, Hashable {
    ...
    public init(maxArchiveSize: UInt = 524288000, maxCompressionRatio: UInt = 100, maxFilesInArchive: UInt = 10000, maxNestingDepth: UInt = 1024, maxEntityLength: UInt = 1048576, maxContentSize: UInt = 104857600, maxIterations: UInt = 10000000, maxXmlDepth: UInt = 1024, maxTableCells: UInt = 100000) {
...
public struct KeywordConfig: Codable, Sendable, Hashable {
    ...
    public init(algorithm: KeywordAlgorithm, maxKeywords: UInt = 10, minScore: Float = 0.0, language: String? = "en", yakeParams: YakeParams? = nil, rakeParams: RakeParams? = nil) {
...
internal extension DoctorCheck {
    init(_ rb: RustBridge.DoctorCheckRef) throws {
        self.name = rb.name().toString()
        self.status = try { let rawValue = rb.status().toString(); guard let value = ProbeStatus(rawValue: rawValue) else { throw XbergError.validation(message: "Unknown ProbeStatus variant", source: rawValue) }; return value }()
        self.message = rb.message().toString()
    }
...
public enum BreadcrumbTarget: String, Codable, Sendable, Hashable {
...
extension BreadcrumbTarget {
    func intoRust() throws -> RustBridge.BreadcrumbTarget {
        let data = try JSONEncoder().encode(self)
        let json = String(data: data, encoding: .utf8) ?? "null"
        return try RustBridge.breadcrumbTargetFromJson(json)
    }
}
...
public typealias ConfidenceSemantics = RustBridge.ConfidenceSemantics
...
public typealias PageOrientationHandling = RustBridge.PageOrientationHandling
...
public func breadcrumbTargetFromJson(_ json: String) throws -> BreadcrumbTarget {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(BreadcrumbTarget.self, from: data)
}
...
/// ```
public func layoutWastesPlainOutput(layoutEnabled: Bool, outputFormat: OutputFormat) throws -> Bool {
        let _rb_outputFormat = try outputFormat.intoRust()
    return RustBridge.layoutWastesPlainOutput(layoutEnabled, _rb_outputFormat)
}
...
public func ensureInitialized() throws {
    return try RustBridge.ensureInitialized()
}
...
public func installPdfRenderDiagnostics() -> Bool {
    return RustBridge.installPdfRenderDiagnostics()
}
...
public func takePdfOxideRenderWarnings() throws -> [ProcessingWarning] {
    return try RustBridge.takePdfOxideRenderWarnings().map { ref in try ProcessingWarning(ref) }
}
...
public func buildDecoderPromptTokens(startOfTranscript: UInt32, langId: UInt32, transcribe: UInt32, noTimestamps: UInt32, timestamps: Bool) -> [Int64] {
    return RustBridge.buildDecoderPromptTokens(startOfTranscript, langId, transcribe, noTimestamps, timestamps).map { $0 }
}
...
public func timestampTokenToMs(tokenId: UInt32, timestampBeginId: UInt32) -> UInt32 {
    return RustBridge.timestampTokenToMs(tokenId, timestampBeginId)
}
