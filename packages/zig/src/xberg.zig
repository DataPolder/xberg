...
// alef:hash:90916ace65941aaebf227b57382aa9101296c861af12f57a9532a0725d6f4f52
// alef:handle-abi:1
...
pub fn layout_wastes_plain_output(layout_enabled: bool, output_format: OutputFormat) error{OutOfMemory}!bool {
    const _result = c.xberg_layout_wastes_plain_output(layout_enabled, output_format);
    return _result != 0;
}
...
pub fn take_xberg_native_pdf_render_warnings() error{OutOfMemory}![]u8 {
    const _result = c.xberg_take_xberg_native_pdf_render_warnings();
    const _result_len = c.xberg_take_xberg_native_pdf_render_warnings_len();
    return blk: {
        const slice = _result[0.._result_len];
        defer _free_string(_result);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
