...
defmodule Xberg do
  ...
  def doctor(config) do
    case Xberg.Native.doctor((cond do is_nil(config) -> nil; is_binary(config) -> config; true -> Jason.encode!(config) end)) do
      {:ok, value} -> value
      {:error, error} -> raise ArgumentError, error
    end
  end
  ...
  @doc "Install the glyph-drop capture as the process-wide `tracing` [`Subscriber`](tracing::Subscriber), exactly once."
  ...
  def max_sim_rank(query, docs) do
    case Xberg.Native.max_sim_rank(query, (cond do is_nil(docs) -> nil; is_binary(docs) -> docs; true -> Jason.encode!(docs) end)) do
      {:ok, value} -> value
      {:error, error} -> raise ArgumentError, error
    end
  end
  ...
  @spec take_xberg_native_pdf_render_warnings() :: [map()]
  def take_xberg_native_pdf_render_warnings do
    Xberg.Native.take_xberg_native_pdf_render_warnings()
  end
  ...
  defmodule OcrBackend.Host do
    ...
    @optional_callbacks [process_image_file: 2, supported_languages: 0, supports_table_detection: 0, supports_document_processing: 0, emits_structured_markdown: 0, confidence_semantics: 0, page_orientation_handling: 0, process_document: 2, initialize: 0, shutdown: 0]
  end
