...
Do not nest these options under `backend_options.sceptre`. Sceptre is available only on native desktop/server hosts
in Xberg 1.1 and requires the `sceptre-ocr` Cargo feature in custom Rust builds.
