...
`output_format` sets the format of the `content` field. It is independent of `result_format` (element-based / document structure). For OCR-backed scanned PDFs, non-`Plain` formats trigger the same geometry-derived structure heuristic (headings, lists) as native PDF extraction; set `output_format` to something other than `Plain` to get structured content instead of raw text.
