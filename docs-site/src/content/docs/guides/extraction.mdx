...
Extract text, metadata, and structure from 106 file formats — PDFs, Office documents, images, email, HTML, archives, and more. Single files or batches, from local paths or in-memory bytes, with per-document configuration overrides and built-in content filtering.
...
Xberg supports 106 file formats across 140 unique file extensions and accepts 53 compatibility MIME aliases:
