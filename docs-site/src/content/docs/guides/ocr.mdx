...
| **Sceptre** | Native CRAFT detection and CRNN recognition; fast CPU inference; six EasyOCR gen2 language groups | Native desktop/server targets only; CPU-only in Xberg 1.1; downloads models on first use | You need low-latency native OCR or EasyOCR-compatible language coverage without a Python runtime |
...
Sceptre is included in published native desktop/server packages and the CLI. For a custom Rust build, enable the
`sceptre-ocr` feature; add `auto-rotate` if you also need automatic orientation correction. Model files download
automatically on first use.

```toml title="Cargo.toml"
[dependencies]
xberg = { version = "1.1", features = ["sceptre-ocr"] }
```

Sceptre is unavailable on WebAssembly, Android, and iOS in Xberg 1.1. Its Xberg integration uses the native ONNX
Runtime backend and is CPU-only.
...
Sceptre 0.1.1 provides English, Latin, Simplified Chinese, Japanese, Korean, and Cyrillic EasyOCR gen2 recognition
groups. Use ISO 639-1 or ISO 639-3 codes, such as `eng`, `deu`, `zho`, `jpn`, `kor`, or `rus`. Incompatible language
group combinations fail validation before OCR starts.
