...
:::note[In progress]
`paddle-ocr-tract` is under active development and not yet released. This section documents the target design.
:::
...
- **Detection runs on a fixed square canvas.** DBNet's shape cannot be left symbolic under tract, so each page is
  padded to a `det_limit_side_len × det_limit_side_len` square (rounded up to a multiple of 32) before detection.
  Aspect ratio is preserved and bounding-box coordinates are unaffected; the cost is computing over the padded area.

Detection cost grows steeply with canvas size. Measured on macOS arm64 (single run, indicative, not a benchmark):
