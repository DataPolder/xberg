...
<!-- alef:hash:eaf97c1c8d3d21af3d028a27ea0c7c0e20433dbabb33b5541088a18d3b1b97e5 -->
...
```go
func TakeXbergNativePdfRenderWarnings() []ProcessingWarning
```

**Example:**

```go
result := TakeXbergNativePdfRenderWarnings()
```
...
| `KClusters` | `uint` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 6, which provides H1-H6 heading levels with body text. Larger values create more fine-grained hierarchy levels. |
...
| `ErrReranking` | The reranker model or reranking pipeline returned an error. Since v5.0. |
| `ErrTranscription` | Audio/video transcription failed. |
| `ErrTimeout` | The extraction operation exceeded the configured time limit. |
| `ErrCancelled` | The extraction was cancelled. Reachable today only via the REST async-jobs API (`DELETE /jobs/{id}`); no binding exposes an in-process way to cancel an extraction. |
