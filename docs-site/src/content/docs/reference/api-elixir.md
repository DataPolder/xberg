...
<!-- alef:hash:947b13ff7339f31ba6173b14557a227ba358e2507a212c9abaae6f69a9c01346 -->
...
```elixir
@spec take_xberg_native_pdf_render_warnings() :: {:ok, term()} | {:error, term()}
def take_xberg_native_pdf_render_warnings()
```

**Example:**

```elixir
{:ok, result} = take_xberg_native_pdf_render_warnings()
```
...
| `backend` | `PdfBackend` | `:native` | Which engine parses and renders this PDF. Defaults to `PdfBackend.Native` -- the only backend with an extraction implementation today. Selecting `PdfBackend.Pdfium` requires the `pdf-pdfium` feature and is rejected otherwise; see `PdfBackend`. |
...
| `language_model_ngram_on` | `boolean()` | `true` | Enable N-gram language model. Kept on by default (see `Self.default` and `language_model_ngram_on` for the rationale); keep this field's default in sync with the internal struct's. |
