...
Xberg supports 104 file formats across 125 file extensions across major categories through native Rust extractors.
...
| SQLite | `.sqlite`, `.db` | `application/vnd.sqlite3`                       | Native Rust (`sqlite`)   | No          | User tables; internal metadata excluded; BLOB size placeholders |
| GeoPackage | `.gpkg`     | `application/geopackage+sqlite3`                  | Native Rust (`sqlite`)   | No          | Schema-based detection; user tables without GeoPackage metadata |
