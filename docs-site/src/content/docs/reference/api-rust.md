...
```rust
pub fn ensure_initialized() -> Result<(), HwpError>
```
...
```rust
pub fn clear_embedding_backends() -> Result<(), HwpError>
```
...
```rust
pub fn list_embedding_backends() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn register_document_extractor(extractor: DocumentExtractor) -> Result<(), HwpError>
```
...
```rust
pub fn unregister_document_extractor(name: &str) -> Result<(), HwpError>
```
...
```rust
pub fn list_document_extractors() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn clear_document_extractors() -> Result<(), HwpError>
```
...
```rust
pub fn list_ocr_backends() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn clear_ocr_backends() -> Result<(), HwpError>
```
...
```rust
pub fn list_post_processors() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn clear_post_processors() -> Result<(), HwpError>
```
...
```rust
pub fn register_renderer(renderer: Renderer) -> Result<(), HwpError>
```
...
```rust
pub fn unregister_renderer(name: &str) -> Result<(), HwpError>
```
...
```rust
pub fn list_renderers() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn clear_renderers() -> Result<(), HwpError>
```
...
```rust
pub fn clear_reranker_backends() -> Result<(), HwpError>
```
...
```rust
pub fn list_reranker_backends() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn clear_tokenizer_backends() -> Result<(), HwpError>
```
...
```rust
pub fn list_tokenizer_backends() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn list_validators() -> Result<Vec<String>, HwpError>
```
...
```rust
pub fn clear_validators() -> Result<(), HwpError>
```
...
```rust
pub async fn classify_chunks(result: ExtractedDocument, config: ChunkClassificationConfig) -> Result<(), HwpError>
```
...
```rust
pub async fn embed_sparse_async(texts: Vec<String>, config: SparseEmbeddingConfig) -> Result<Vec<SparseEmbedding>, HwpError>
```
...
```rust
pub async fn embed_multi_vector_async(texts: Vec<String>, config: LateInteractionConfig, is_query: bool) -> Result<Vec<MultiVectorEmbedding>, HwpError>
```
...
```rust
pub async fn embed_sparse_async(texts: Vec<String>, config: SparseEmbeddingConfig) -> Result<Vec<SparseEmbedding>, PdfError>
```
...
```rust
pub async fn embed_multi_vector_async(texts: Vec<String>, config: LateInteractionConfig, is_query: bool) -> Result<Vec<MultiVectorEmbedding>, PdfError>
```
...
| `breadcrumb_target` | `BreadcrumbTarget` | `BreadcrumbTarget::Content` | Where the heading-path breadcrumb is written when `prepend_heading_context` is `true`. See `BreadcrumbTarget` for the dense-vs-lexical retrieval trade-off. Has no effect when `prepend_heading_context` is `false`. Default: `Content` — preserves the pre-existing behaviour of prepending the breadcrumb into chunk `content`. |
...
```rust
pub async fn extract(&self, input: ExtractInput, config: ExtractionConfig) -> Result<ExtractedDocument, HwpError>
```
...
```rust
pub async fn embed(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>, HwpError>
```
...
```rust
pub async fn process_image(&self, image_bytes: &[u8], config: OcrConfig) -> Result<ExtractedDocument, HwpError>
```
...
```rust
pub async fn process_image_file(&self, path: PathBuf, config: OcrConfig) -> Result<ExtractedDocument, HwpError>
```
...
```rust
pub async fn process_document(&self, path: PathBuf, config: OcrConfig) -> Result<ExtractedDocument, HwpError>
```
...
```rust
pub fn initialize(&self) -> Result<(), HwpError>
```
...
```rust
pub fn shutdown(&self) -> Result<(), HwpError>
```
...
```rust
pub async fn process(&self, result: ExtractedDocument, config: ExtractionConfig) -> Result<(), HwpError>
```
...
```rust
pub fn render_result(&self, result: ExtractedDocument) -> Result<String, HwpError>
```
...
```rust
pub async fn rerank(&self, query: &str, documents: Vec<String>) -> Result<Vec<f32>, HwpError>
```
...
```rust
pub async fn validate(&self, result: ExtractedDocument, config: ExtractionConfig) -> Result<(), HwpError>
```
...
| Value | Description |
|-------|-------------|
| `Content` | Prepend the heading breadcrumb into chunk `content`, and keep `heading_path` populated (default; preserves the behaviour that existed before this option was introduced). |
| `Metadata` | Leave `content` untouched; rely on `ChunkMetadata::heading_path` only. |
