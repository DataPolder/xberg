...
<!-- alef:hash:3018533866c618a4a5fea98e4f3755ccf067567166848bb0a04713bb5d92b257 -->
...
```python
result = take_xberg_native_pdf_render_warnings()
```
...
| `RerankingError(XbergError)` | The reranker model or reranking pipeline returned an error. Since v5.0. |
...
| `CancelledError(XbergError)` | The extraction was cancelled. Reachable today only via the REST async-jobs API (`DELETE /jobs/{id}`); no binding exposes an in-process way to cancel an extraction. |
