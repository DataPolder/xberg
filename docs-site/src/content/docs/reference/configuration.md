...
| Variant | Wire value | Description |
|---------|------------|-------------|
| `Content` | `content` | Prepend the heading breadcrumb into chunk `content`, and keep `heading_path` populated (default; preserves the behaviour that existed before this option was introduced). |
| `Metadata` | `metadata` | Leave `content` untouched; rely on `ChunkMetadata.heading_path` only. |
