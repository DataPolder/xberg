...
<!-- alef:hash:dd32bd77969169a64728e28f73422fbf022e4f33015349d10f017b481b34bfe1 -->
...
```java
public static void classifyChunks(ExtractedDocument result, ChunkClassificationConfig config) throws XbergRsException
```
...
#### takeXbergNativePdfRenderWarnings()
...
```java
var result = takeXbergNativePdfRenderWarnings();
```
...
```java
public static List<Long> buildDecoderPromptTokens(int startOfTranscript, int langId, int transcribe, int noTimestamps, boolean timestamps) throws XbergRsException
```
...
```java
public static int timestampTokenToMs(int tokenId, int timestampBeginId) throws XbergRsException
```
...
| `parentId` | `@Nullable String` | `null` | Parent element ID for hierarchical relationships. Only used for Tesseract output which has word -> line -> block hierarchy. |
...
| `topMarginFraction` | `@Nullable Float` | `null` | Top margin fraction (0.0–1.0) of page height to exclude headers/running heads. Default: 0.06 (6%) |
| `bottomMarginFraction` | `@Nullable Float` | `null` | Bottom margin fraction (0.0–1.0) of page height to exclude footers/page numbers. Default: 0.05 (5%) |
...
| `languageModelNgramOn` | `boolean` | `true` | Enable N-gram language model. Kept on by default (see `Self.default` and `language_model_ngram_on` for the rationale); keep this field's default in sync with the internal struct's. |
...
| `RerankingException` | The reranker model or reranking pipeline returned an error. Since v5.0. |
...
| `CancelledException` | The extraction was cancelled. Reachable today only via the REST async-jobs API (`DELETE /jobs/{id}`); no binding exposes an in-process way to cancel an extraction. |
