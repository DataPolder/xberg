...
<!-- alef:hash:8ab09b774835ab77a4fac93d783bb4fb504f8cf298630745e5efebdc7817bcc4 -->
...
```c
XBERGAlefHandle xberg_extract_batch(XBERGAlefHandle* inputs, XBERGAlefHandle config);
```
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `inputs` | `XBERGAlefHandle*` | Yes | The inputs |
| `config` | `XBERGAlefHandle` | Yes | The configuration options |
...
```c
XBERGAlefHandle* xberg_list_supported_formats();
```

**Example:**

```c
XBERGAlefHandle* result = xberg_list_supported_formats();
```
...
```c
int32_t xberg_ensure_initialized();
```
...
```c
XBERGAlefHandle* xberg_embed_sparse_async(const char** texts, XBERGAlefHandle config);
```

**Example:**

```c
XBERGAlefHandle* result = xberg_embed_sparse_async(NULL, 0);
```
...
```c
XBERGAlefHandle* xberg_max_sim_rank(XBERGAlefHandle query, XBERGAlefHandle* docs);
```

**Example:**

```c
XBERGAlefHandle* result = xberg_max_sim_rank(0, NULL);
```
...
|------|------|----------|-------------|
| `query` | `XBERGAlefHandle` | Yes | The multi vector embedding |
| `docs` | `XBERGAlefHandle*` | Yes | The docs |
...
```c
XBERGAlefHandle* xberg_embed_multi_vector_async(const char** texts, XBERGAlefHandle config, int32_t is_query);
```

**Example:**

```c
XBERGAlefHandle* result = xberg_embed_multi_vector_async(NULL, 0, true);
```
...
| `texts` | `const char**` | Yes | The texts |
| `config` | `XBERGAlefHandle` | Yes | The configuration options |
| `is_query` | `int32_t` | Yes | The is query |
...
```c
int32_t xberg_install_pdf_render_diagnostics();
```

**Example:**

```c
int32_t result = xberg_install_pdf_render_diagnostics();
```
...
#### xberg_take_xberg_native_pdf_render_warnings()
...
```c
XBERGAlefHandle* xberg_take_xberg_native_pdf_render_warnings();
```

**Example:**

```c
XBERGAlefHandle* result = xberg_take_xberg_native_pdf_render_warnings();
```
...
```c
int64_t* xberg_build_decoder_prompt_tokens(uint32_t start_of_transcript, uint32_t lang_id, uint32_t transcribe, uint32_t no_timestamps, int32_t timestamps);
```
...
| `timestamps` | `int32_t` | Yes | The timestamps |
...
| `definitions` | `XBERGAlefHandle*` | — | The set of label definitions the classifier may emit. Must contain at least one entry. |
...
| `breadcrumb_target` | `XBERGAlefHandle` | `XBERG_XBERG_CONTENT` | **Deprecated and inert** (#1393): see `BreadcrumbTarget` for the full explanation. Neither variant has any effect on `content` any more — call `render_heading_breadcrumb` explicitly at index time instead. Kept only for backward compatibility. Default: `Content`. |
...
| `attributes` | `XBERGAlefHandle*` | `/* serde(default) */` | Attributes on element-shape nodes (XML `STag` attributes). Empty for all other kinds. |
| `children` | `XBERGAlefHandle*` | `/* serde(default) */` | Children for nested containers and XML element bodies. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `chunks` | `XBERGAlefHandle*` | `NULL` | Structural code chunks (function/class/module boundaries). |
...
| `asset_types` | `XBERGAlefHandle*` | `NULL` | Filter for asset categories to download. |
...
| `fields` | `XBERGAlefHandle*` | `NULL` | Descriptor for each field in the table schema. |
...
| `detections` | `XBERGAlefHandle*` | — | All layout detections on this page after postprocessing. |
...
| `lines` | `XBERGAlefHandle*` | — | Lines that make up this hunk. |
...
| `blocks` | `XBERGAlefHandle*` | — | Structured block-level content |
| `metadata` | `XBERGAlefHandle` | — | Metadata from YAML frontmatter |
| `tables` | `XBERGAlefHandle*` | — | Extracted tables as structured data |
| `images` | `XBERGAlefHandle*` | — | Extracted images with metadata |
| `links` | `XBERGAlefHandle*` | — | Extracted links with URLs |
| `footnotes` | `XBERGAlefHandle*` | — | Footnote definitions |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `checks` | `XBERGAlefHandle*` | `NULL` | Individual check verdicts, in execution order. |
...
| `annotations` | `XBERGAlefHandle*` | `/* serde(default) */` | Inline annotations (formatting, links) on this node's text content. Only meaningful for text-carrying nodes; empty for containers. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `nodes` | `XBERGAlefHandle*` | `NULL` | All nodes in document/reading order. |
...
| `relationships` | `XBERGAlefHandle*` | `NULL` | Resolved relationships between nodes (footnote refs, citations, anchor links, etc.). Populated during derivation from the internal document representation. Empty when no relationships are detected. |
...
| `attachments` | `XBERGAlefHandle*` | — | List of email attachments |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `added` | `XBERGAlefHandle*` | `NULL` | Children present in `b` but not in `a` (matched by `path`). |
| `removed` | `XBERGAlefHandle*` | `NULL` | Children present in `a` but not in `b` (matched by `path`). |
| `changed` | `XBERGAlefHandle*` | `NULL` | Children present in both but with differing content (matched by `path`). Each entry holds the diff of the nested `ExtractedDocument`. |
...
```c
float** xberg_embedding_backend_embed(XBERGAlefHandle this, const char** texts);
```

**Example:**

```c
float** result = xberg_embedding_backend_embed(instance, NULL);
```
...
| `tables` | `XBERGAlefHandle*` | `NULL` | Tables extracted from the document, each with structured cell data. |
...
| `detected_language_confidences` | `XBERGAlefHandle*` | `NULL` | Structured per-language detection results: confidence, document share, script, and reliability, alongside the ISO-code-only `detected_languages` (#261). One entry per language in `detected_languages`, in the same order. `NULL` under the same conditions as `detected_languages`: detection disabled, empty input text, or no language met the configured `min_confidence`. |
| `chunks` | `XBERGAlefHandle*` | `NULL` | Text chunks when chunking is enabled. When chunking configuration is provided, the content is split into overlapping chunks for efficient processing. Each chunk contains the text, optional embeddings (if enabled), and metadata about its position. |
| `images` | `XBERGAlefHandle*` | `NULL` | Extracted images from the document. When image extraction is enabled via `ImageExtractionConfig`, this field contains all images found in the document with their raw data and metadata. Each image may optionally contain a nested `ocr_result` if OCR was performed. |
| `pages` | `XBERGAlefHandle*` | `NULL` | Per-page content when page extraction is enabled. When page extraction is configured, the document is split into per-page content with tables and images mapped to their respective pages. |
| `elements` | `XBERGAlefHandle*` | `NULL` | Semantic elements when element-based result format is enabled. When result_format is set to ElementBased, this field contains semantic elements with type classification, unique identifiers, and metadata for Unstructured-compatible element-based processing. |
...
| `ocr_elements` | `XBERGAlefHandle*` | `NULL` | OCR elements with full spatial and confidence metadata. When OCR is performed with element extraction enabled, this field contains the structured representation of detected text including: - Bounding geometry (rectangles or quadrilaterals) - Confidence scores (detection and recognition) - Rotation information - Hierarchical relationships (Tesseract only) This field preserves all metadata that would otherwise be lost when converting to plain text or markdown output formats. Only populated when `OcrElementConfig.include_elements` is true. |
...
| `extracted_keywords` | `XBERGAlefHandle*` | `NULL` | Extracted keywords when keyword extraction is enabled. When keyword extraction (RAKE or YAKE) is configured, this field contains the extracted keywords with scores, algorithm info, and position data. Previously stored in `metadata.additional["keywords"]`. |
...
| `processing_warnings` | `XBERGAlefHandle*` | `NULL` | Non-fatal warnings collected during processing pipeline stages. Captures errors from optional pipeline features (embedding, chunking, language detection, output formatting) that don't prevent extraction but may indicate degraded results. Previously stored as individual keys in `metadata.additional`. |
| `annotations` | `XBERGAlefHandle*` | `NULL` | PDF annotations extracted from the document. When annotation extraction is enabled via `PdfConfig.extract_annotations`, this field contains text notes, highlights, links, stamps, and other annotations found in PDF documents. |
| `children` | `XBERGAlefHandle*` | `NULL` | Nested extraction results from archive contents. When extracting archives, each processable file inside produces its own full extraction result. Set to `NULL` for non-archive formats. Use `max_archive_depth` in config to control recursion depth. |
| `uris` | `XBERGAlefHandle*` | `NULL` | URIs/links discovered during document extraction. Contains hyperlinks, image references, citations, email addresses, and other URI-like references found in the document. Always extracted when present in the source document. |
| `revisions` | `XBERGAlefHandle*` | `NULL` | Tracked changes embedded in the source document. Populated by per-format extractors that understand change-tracking metadata (DOCX `w:ins`/`w:del`/`w:rPrChange`, ODT `text:change-*`, …). Every extractor defaults to `NULL` until its format-specific implementation is added. Extractors that do populate this field follow the "accepted-changes" convention: inserted text is present in `content`, deleted text is absent — the revision list is the separate audit trail. |
...
| `llm_usage` | `XBERGAlefHandle*` | `NULL` | LLM token usage and cost data for all LLM calls made during this extraction. Contains one entry per LLM call. Multiple entries are produced when VLM OCR, structured extraction, or LLM embeddings run during the same extraction. `NULL` when no LLM was used. |
| `entities` | `XBERGAlefHandle*` | `NULL` | Named entities detected in `content` by the NER post-processor. `NULL` when no NER backend is configured. Populated by the `xberg-gliner` ONNX backend or the LLM-driven backend (see `crates/xberg/src/text/ner/`). |
...
| `page_classifications` | `XBERGAlefHandle*` | `NULL` | Per-page classifications produced by the page-classification post-processor. `NULL` when classification is not configured. |
...
| `formulas` | `XBERGAlefHandle*` | `NULL` | Mathematical formulas recognized in the document. Populated from every source that produces formulas: layout-guided OCR (with geometry), VLM OCR (text only), and markup extraction (DOCX, PPTX, ODT, EPUB, HTML, JATS, LaTeX, Markdown, and related formats, without geometry). Empty when the document contains no formulas. |
| `form_fields` | `XBERGAlefHandle*` | `NULL` | Form fields extracted from a PDF's AcroForm or XFA structure. Populated by the PDF extractor when `PdfConfig.extract_form_fields` is enabled (default) and the document is a fillable form. Empty otherwise. |
...
| `qr_codes` | `XBERGAlefHandle*` | `NULL` | QR codes decoded from this image, when QR detection is enabled. Populated by the QR post-processor (`crates/xberg/src/extractors/qr.rs`) via the pure-Rust `rqrr` decoder. `NULL` when QR detection is disabled; an empty `Some([])` when detection ran but found nothing. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content_diff` | `XBERGAlefHandle*` | `NULL` | Unified-diff hunks for the `content` field. Empty when the content is identical. |
| `tables_added` | `XBERGAlefHandle*` | `NULL` | Tables present in `b` but not in `a` (by index position, excess right-side tables). |
| `tables_removed` | `XBERGAlefHandle*` | `NULL` | Tables present in `a` but not in `b` (by index position, excess left-side tables). |
| `tables_changed` | `XBERGAlefHandle*` | `NULL` | Cell-level changes for table pairs that share the same index and dimensions. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `XBERGAlefHandle*` | `NULL` | Extracted documents in discovery order. |
| `errors` | `XBERGAlefHandle*` | `NULL` | Non-fatal per-input errors. |
...
| `content` | `XBERGAlefHandle*` | — | Footnote content blocks |
...
| `inline_content` | `XBERGAlefHandle*` | — | Inline content within the block |
| `language` | `const char*` | `NULL` | Language identifier for code blocks |
| `code` | `const char*` | `NULL` | Raw code content for code blocks |
| `children` | `XBERGAlefHandle*` | `/* serde(default) */` | Nested blocks for containers (blockquotes, list items, divs) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `headings` | `XBERGAlefHandle*` | — | The heading hierarchy from document root to this chunk's section. Index 0 is the outermost (h1), last element is the most specific. |
...
| `headers` | `XBERGAlefHandle*` | `NULL` | Extracted header elements with hierarchy |
| `links` | `XBERGAlefHandle*` | `NULL` | Extracted hyperlinks with type classification |
| `images` | `XBERGAlefHandle*` | `NULL` | Extracted images with source and dimensions |
| `structured_data` | `XBERGAlefHandle*` | `NULL` | Extracted structured data blocks |
...
| `contributor_roles` | `XBERGAlefHandle*` | `NULL` | Authors and contributors with their stated roles. |
...
| `providers` | `XBERGAlefHandle*` | `NULL` | Custom provider configurations, in addition to liter-llm's built-in providers. Mirrors liter-llm's `LlmConfig.providers`, for OpenAI-compatible gateways and self-hosted model servers that are not in the built-in provider catalog. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `urls` | `XBERGAlefHandle*` | `NULL` | The list of discovered URLs. |
...
| `pages` | `XBERGAlefHandle*` | — | Per-page signals extracted from the PDF. |
...
| `categories` | `XBERGAlefHandle*` | `NULL` | Entity categories to detect. Defaults to a sensible PERSON/ORG/LOCATION/EMAIL set when empty. |
...
| `build_hierarchy` | `int32_t` | — | Whether to build hierarchical relationships between elements. When true, `parent_id` fields will be populated based on spatial containment. Only meaningful for Tesseract output. |
...
| `tables` | `XBERGAlefHandle*` | `NULL` | Tables detected and extracted via OCR |
| `ocr_elements` | `XBERGAlefHandle*` | `NULL` | Structured OCR elements with bounding boxes and confidence scores. Available when TSV output is requested or table detection is enabled. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `stages` | `XBERGAlefHandle*` | — | Ordered list of backends to try. Sorted by priority (descending) at runtime. |
...
| `labels` | `XBERGAlefHandle*` | — | Labels assigned to the page. Single-label classification yields exactly one entry; multi-label classification yields any subset of the configured label set. |
...
| `tables` | `XBERGAlefHandle*` | `/* serde(default) */` | Tables found on this page (uses Arc for memory efficiency) Serializes as `const Table*` for JSON compatibility while maintaining shared in-memory ownership for zero-copy sharing. |
...
| `layout_regions` | `XBERGAlefHandle*` | `NULL` | Layout detection regions for this page (when layout detection is enabled). Contains detected layout regions with class, confidence, bounding box, and area fraction. Only populated when layout detection is configured. |
...
| `blocks` | `XBERGAlefHandle*` | `/* serde(default) */` | Hierarchical blocks with heading levels |
...
| `boundaries` | `XBERGAlefHandle*` | `NULL` | Character offset boundaries for each page Maps character ranges in the extracted content to page numbers. Used for chunk page range calculation. |
| `pages` | `XBERGAlefHandle*` | `NULL` | Detailed per-page metadata (optional, only when needed) |
...
| `quad_points` | `XBERGAlefHandle*` | `/* serde(default) */` | Per-line bounding boxes derived from the annotation's `/QuadPoints` entry. Present for text markup annotations (Highlight, Underline, StrikeOut, Squiggly), one box per marked line/run of text. |
...
| `ocr_inline_images` | `int32_t` | `false` | Perform OCR on inline images extracted from PDF pages and attach the recognized text to each `ExtractedImage.ocr_result`. Requires Tesseract to be available; if `ExtractionConfig.ocr` is `NULL` the extractor falls back to `TesseractConfig.default()`. Per-image failures degrade gracefully (the image is returned without OCR text rather than failing the whole extraction). Default: `false`. |
...
| `backend` | `XBERGAlefHandle` | `XBERG_XBERG_NATIVE` | Which engine parses and renders this PDF. Defaults to `PdfBackend.Native` -- the only backend with an extraction implementation today. Selecting `PdfBackend.Pdfium` requires the `pdf-pdfium` feature and is rejected otherwise; see `PdfBackend`. |
...
| `images` | `XBERGAlefHandle*` | — | Extracted images from the presentation |
| `page_structure` | `XBERGAlefHandle` | `NULL` | Slide structure with boundaries (when page tracking is enabled) |
| `page_contents` | `XBERGAlefHandle*` | `NULL` | Per-slide content (when page tracking is enabled) |
...
| `revisions` | `XBERGAlefHandle*` | `/* serde(default) */` | Slide comments as revisions. Each `<p:cm>` element in `ppt/comments/comment{N}.xml` becomes a `DocumentRevision { kind: Comment }` with author (resolved from `ppt/commentAuthors.xml`), ISO-8601 timestamp, and `RevisionAnchor.Slide { index }`. `NULL` when no comment XML parts exist. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `categories` | `XBERGAlefHandle*` | `NULL` | Categories to redact. Empty means "every category supported by the engine." |
...
| `custom_terms` | `XBERGAlefHandle*` | `NULL` | Arbitrary user-supplied literal terms to redact. Each term is treated as a regex hit against the document, surfacing as `PiiCategory.Custom(label)` in `RedactionFinding` where `label` is the per-term label (defaulting to the literal value itself). Case-insensitive by default; set `RedactionTerm.case_sensitive` for exact match. Use this when you need to redact tenant-specific tokens (employee IDs, project codes, internal product names) without writing a custom plugin. |
| `custom_patterns` | `XBERGAlefHandle*` | `NULL` | Arbitrary user-supplied regex patterns to redact. Same surfacing semantics as `custom_terms`: each hit becomes a `PiiCategory.Custom(label)` finding. Patterns are validated at config-construction time via `RedactionConfig.validate`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `findings` | `XBERGAlefHandle*` | — | Individual redaction findings in original-source byte order. |
...
```c
XBERGAlefHandle* xberg_registry_summaries(XBERGAlefHandle this);
```

**Example:**

```c
XBERGAlefHandle* result = xberg_registry_summaries(instance);
```
...
```c
float* xberg_reranker_backend_rerank(XBERGAlefHandle this, const char* query, const char** documents);
```

**Example:**

```c
float* result = xberg_reranker_backend_rerank(instance, "value", NULL);
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `content` | `XBERGAlefHandle*` | `NULL` | Line-level content changes for this revision. |
| `table_changes` | `XBERGAlefHandle*` | `NULL` | Cell-level table changes for this revision. |
| `property_changes` | `XBERGAlefHandle*` | `NULL` | Formatting or metadata property changes for this revision. |
...
| `cell_changes` | `XBERGAlefHandle*` | — | Cell-level changes within the table. |
...
| `cells` | `XBERGAlefHandle*` | `NULL` | All cells in row-major order. |
...
| `language_model_ngram_on` | `int32_t` | `true` | Enable N-gram language model. Kept on by default (see `Self.default` and `language_model_ngram_on` for the rationale); keep this field's default in sync with the internal struct's. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `page_ranges` | `XBERGAlefHandle*` | `NULL` | User-specified page ranges (overrides automatic chunking). |
...
| `XBERG_CANCELLED` | The extraction was cancelled. Reachable today only via the REST async-jobs API (`DELETE /jobs/{id}`); no binding exposes an in-process way to cancel an extraction. |
