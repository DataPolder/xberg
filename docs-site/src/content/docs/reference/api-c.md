...
| `breadcrumb_target` | `XbergBreadcrumbTarget` | `XBERG_XBERG_CONTENT` | Where the heading-path breadcrumb is written when `prepend_heading_context` is `true`. See `BreadcrumbTarget` for the dense-vs-lexical retrieval trade-off. Has no effect when `prepend_heading_context` is `false`. Default: `Content` — preserves the pre-existing behaviour of prepending the breadcrumb into chunk `content`. |
...
| Value | Description |
|-------|-------------|
| `XBERG_CONTENT` | Prepend the heading breadcrumb into chunk `content`, and keep `heading_path` populated (default; preserves the behaviour that existed before this option was introduced). |
| `XBERG_METADATA` | Leave `content` untouched; rely on `ChunkMetadata.heading_path` only. |
