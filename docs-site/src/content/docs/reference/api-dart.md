...
<!-- alef:hash:f9c2109fe23c733ffe4bdd2d162461503727722f93e2f9bdba2805576d92f3c1 -->
...
```dart
Future<void> classifyChunks(ExtractedDocument result, ChunkClassificationConfig config)
```
...
```dart
final result = takeXbergNativePdfRenderWarnings();
```
...
```dart
Future<List<int>> buildDecoderPromptTokens(int startOfTranscript, int langId, int transcribe, int noTimestamps, bool timestamps)
```
...
```dart
Future<int> timestampTokenToMs(int tokenId, int timestampBeginId)
```
...
| `kClusters` | `PlatformInt64` | `3` | Number of font size clusters to use for hierarchy levels (1-7) Default: 6, which provides H1-H6 heading levels with body text. Larger values create more fine-grained hierarchy levels. |
...
| `languageModelNgramOn` | `bool` | `true` | Enable N-gram language model. Kept on by default (see `Self.default` and `language_model_ngram_on` for the rationale); keep this field's default in sync with the internal struct's. |
