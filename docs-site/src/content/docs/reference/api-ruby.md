...
| `breadcrumb_target` | `BreadcrumbTarget` | `:content` | Where the heading-path breadcrumb is written when `prepend_heading_context` is `true`. See `BreadcrumbTarget` for the dense-vs-lexical retrieval trade-off. Has no effect when `prepend_heading_context` is `false`. Default: `Content` — preserves the pre-existing behaviour of prepending the breadcrumb into chunk `content`. |
