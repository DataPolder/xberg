...
def test_verify_reports_every_stale_publication_phrase() -> None:
    files = [
        ...
        TextFile(
            Path("Cargo.toml"), 'description = "Document extraction from 101 formats"\n'
        ),
    ]
...
def test_synchronize_skips_generated_ownership_and_updates_sources(
    ...
    plugin.parent.mkdir()
    plugin.write_text(
        '{"description":"Xberg extracts 101 formats."}\n', encoding="utf-8"
    )
    ...
    assert changed == [Path("templates/readme/root.md")]
